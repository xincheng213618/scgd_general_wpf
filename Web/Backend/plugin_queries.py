...
def paginate_plugin_summaries(
    ...
    *,
    page: int,
    page_size: int,
) -> tuple[list[PluginItem], int, int, int]:
    items = list(plugins)
    ...
    return paged, total_count, total_pages, current_page
...
def build_plugin_search_result(
    ...
    *,
    ...
    paged, total_count, total_pages, current_page = paginate_plugin_summaries(
        plugins,
        page=page,
        page_size=page_size,
    )
