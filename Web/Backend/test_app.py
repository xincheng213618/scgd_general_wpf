...
class MarketplaceAppTests(unittest.TestCase):
    ...
    def test_site_updates_api_lists_incremental_packages(self):
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        versions = {item["version"] for item in payload["update_packages"]}
        other_names = {item["name"] for item in payload["other_update_items"]}
        self.assertIn("1.0.0.1", versions)
        self.assertIn("1.0.0.3", versions)
        self.assertIn("notes.txt", other_names)

    def test_site_home_api_emphasizes_filesystem_spotlight_and_archive_timeline(self):
        ...
        self.assertIn("ZIP", archive_kinds)

    def test_site_releases_api_returns_grouped_archive_timeline_with_timestamps(self):
        ...
        response = self.client.get("/api/site/releases")

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        groups = payload["archive_visible_groups"]
        self.assertTrue(any(group["branch"] == "1.0.0" for group in groups))
        kinds = {
            item["kind"]
            for group in groups
            for item in group["visible_items"]
        }
        self.assertIn("ZIP", kinds)
        self.assertIn("RAR", kinds)
        modified_values = [
    ...
    def test_site_releases_api_supports_archive_filters(self):
        ...
        response = self.client.get("/api/site/releases?major_minor=1.1&branch=1.1.0&kind=ZIP&era=archive")

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["archive_visible_item_count"], 1)
        self.assertEqual(payload["archive_visible_groups"][0]["branch"], "1.1.0")
        self.assertEqual(payload["archive_visible_groups"][0]["visible_items"][0]["kind"], "ZIP")
        self.assertTrue(payload["archive_visible_groups"][0]["is_expanded"])
        self.assertTrue(payload["release_filters"]["has_filters"])

    def test_site_releases_api_defaults_to_first_stage_expanded(self):
        self._create_app_release("1.2.0.1", suffix=".exe", mtime=1_775_000_000)
        self._create_app_release("1.1.0.1", in_history=True, suffix=".zip", mtime=1_774_000_000)
        self._create_app_release("1.0.0.1", in_history=True, suffix=".exe", mtime=1_773_000_000)
        ...
        self.assertEqual(response.status_code, 200)
        groups = response.get_json()["archive_visible_groups"]
        self.assertEqual(sum(1 for group in groups if group["is_expanded"]), 1)

    def test_site_home_api_returns_recent_change_dashboard(self):
        (self.storage / "History").mkdir(parents=True, exist_ok=True)
        self._create_app_release("1.2.0.1", suffix=".exe", mtime=1_775_000_000)
        self._create_app_release("1.1.0.1", in_history=True, suffix=".zip", mtime=1_774_000_000)
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        titles = {item["title"] for item in payload["recent_change_dashboard"]}
        categories = {item["category"] for item in payload["recent_change_dashboard"]}
    ...
    def test_site_changelog_api_returns_release_history_payload(self):
        ...
        response = self.client.get("/api/site/changelog")

        self.assertEqual(response.status_code, 200)
        app_info = response.get_json()["app_info"]
        self.assertIn("新增插件市场", app_info["changelog"])
        self.assertIn("新增插件市场", app_info["changelog_html"])
        self.assertIn("release_timeline", app_info)

    def test_site_tools_api_lists_tool_directory_contents(self):
        ...
        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertTrue(any(item["name"] == "SomeInstaller.exe" for item in payload["items"]))
    ...
    def test_plugin_list_endpoint_does_not_compute_package_hashes(self):
    ...
    def test_plugin_query_filter_and_sort_contract(self):
    ...
    def test_api_plugins_supports_pagination(self):
        ...
        response = self.client.get("/api/plugins?SortBy=name&SortOrder=asc&Page=2&PageSize=2")

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["page"], 2)
        self.assertEqual(payload["totalPages"], 3)
        self.assertEqual([item["pluginId"] for item in payload["items"]], ["PagePlugin03", "PagePlugin04"])
    ...
    def test_api_plugins_clamps_out_of_range_page_to_last_page(self):
        ...
        response = self.client.get("/api/plugins?SortBy=name&SortOrder=asc&Page=9&PageSize=2")

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload["page"], 2)
        self.assertEqual(payload["totalPages"], 2)
        self.assertEqual([item["pluginId"] for item in payload["items"]], ["ClampPlugin03"])
