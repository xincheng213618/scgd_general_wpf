...
def build_updates_page_context(storage: Path, *, cache_manager=None) -> dict[str, Any]:
    def public_update_package(item: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "relative_path": item.get("relative_path", f"Update/{item.get('filename', '')}"),
        }
    ...
    public_packages = [public_update_package(item) for item in canonical_packages]
    other_update_items = [
        {
            "name": item["filename"],
            "is_dir": False,
    ...
    return {
        "update_packages": public_packages,
        "other_update_files": other_files,
        "other_update_items": other_update_items,
        "update_summary": build_update_summary(canonical_packages, other_files),
        "retention_note": RETENTION_NOTE,
