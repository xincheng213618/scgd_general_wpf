...
ColrVision은 귀하에게 다음과 같은 비독점적이고 양도 불가능한 권리를 부여합니다.
- 귀하가 소유하거나 제어하는 단일 컴퓨터에 소프트웨어 사본 하나를 설치하고 사용합니다.
- 백업 또는 보관 목적으로만 소프트웨어의 백업 복사본 하나를 만드는 것.

**2. 라이센스 제한**

본 계약에서 명시적으로 허용하지 않는 한 귀하는 다음을 수행할 수 없습니다.
- 소프트웨어의 소스 코드를 복사, 수정, 번역, 디컴파일, 리버스 엔지니어링, 분해 또는 기타 방법으로 검색하려고 시도합니다.
- 소프트웨어 또는 본 계약에 의해 부여된 모든 권리를 임대, 임대, 대여, 양도 또는 기타 방식으로 양도합니다.
- 소프트웨어에 포함된 저작권 또는 기타 소유권 고지를 제거하거나 변경합니다.

**3. 소유권**

소프트웨어는 ColrVision이 소유하고 운영합니다. 귀하에게는 소프트웨어 소유권이 아닌 소프트웨어 사용에 대한 제한된 라이센스가 부여됩니다. 소프트웨어 및 모든 사본에 대한 모든 권리, 소유권 및 이권은 ColrVision의 독점 자산이며 앞으로도 그럴 것입니다.

**4. 종료**

귀하가 해당 조항을 준수하지 않을 경우 본 계약은 자동으로 종료됩니다. 종료 시 귀하는 즉시 소프트웨어 사용을 중단하고 소프트웨어의 모든 사본을 파기해야 합니다.

**5. 보증 및 면책조항**

소프트웨어는 어떠한 종류의 보증도 없이 "있는 그대로" 제공됩니다. ColrVision은 상품성, 특정 목적에의 적합성 또는 비침해에 대한 보증을 포함하되 이에 국한되지 않는 어떠한 명시적 또는 묵시적 보증도 하지 않습니다. ColrVision은 소프트웨어에 오류가 없거나 중단되지 않을 것이라고 보증하지 않습니다.

**6. 책임의 한계**

어떠한 경우에도 ColrVision은 소프트웨어 사용 또는 사용 불능으로 인해 발생하는 간접적, 특별, 우발적, 징벌적 또는 결과적 손해(이익 손실, 비즈니스 중단 또는 정보 손실을 포함하되 이에 국한되지 않음)에 대해 책임을 지지 않습니다. 이는 ColrVision이 그러한 손해의 가능성을 통보받은 경우에도 마찬가지입니다.

**7. 준거법**

본 계약은 국제사법 원칙에 관계없이 중화인민공화국 법률에 따라 규율되고 해석됩니다. 본 계약으로 인해 또는 본 계약과 관련하여 발생하는 모든 분쟁은 ColrVision이 위치한 관할권의 관할 법원에 제출됩니다.

**8. 전체 계약**

본 계약은 본 계약의 주제와 관련된 당사자 간의 완전한 합의를 구성하며 이전 및 현재의 모든 계약, 진술 및 이해를 대체합니다.

**9. 수정 및 면제**

본 계약에 대한 모든 수정 또는 포기는 서면으로 이루어져야 하며 양 당사자의 승인된 대표자가 서명해야 효력이 발생합니다.

**10. 분리성**

본 계약의 어떤 조항이 유효하지 않거나 시행할 수 없는 경우에도 나머지 조항은 완전한 효력을 유지합니다.
...
**중요 사건:**本ソフトウェア使用許諾契約（以下「契約」といいます）は、ColrVision Inc（以下「ColrVision」といいます）が開発したWindows WPFソフトウェア（以下「ソフトウェア」といいます）に関する、あなた（個人または単一の法人）と視彩（上海）光電技術有限公司（以下「視彩」といいます）との間の法的契約です。本ソフトウェアをインストール、コピー、またはその他の方法で使用することにより、あなたは本契約の条項に拘束されることに同意したものとみなされます。本契約の条項に同意しない場合は、本ソフトウェアをインストールまたは使用しないでください。
