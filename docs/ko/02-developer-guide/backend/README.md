...
- **React Web 포털** - 찾아보기, 검색, 다운로드, 게시 및 운영 관리
...
```
Web/Backend/
├── app.py # Flask 애플리케이션 메인 입구 (React SPA + API + 파일 서비스)
...
├── routes/ # 사이트 데이터, 인증, 관리 API, 파일 서비스 블루프린트
└── services/ # 인덱싱, 인증, 스케줄러 및 스토리지 이벤트 서비스
```
## 설치 및 실행
...
### 빌드 스크립트 및 클라이언트 다운로드 라우팅| 라우팅 모드 | 설명 |
|------------|------|
| `PUT /upload/{path}` | 빌드 스크립트 직접 산출물 업로드 |
| `/D%3A/ColorVision/Plugins/{path}` | 데스크톱 클라이언트 버전 확인 및 다운로드 |
...
| 언어 | 파이썬 | 3.9+ |
| 프레임워크 | 플라스크 | >=3.0 |
| 프런트엔드 | React + TypeScript + Ant Design | `Web/Frontend` 참고 |
| 데이터베이스 | SQLite | 내장 |
| 마크다운 렌더링 | 인하 | >=3.8 |
