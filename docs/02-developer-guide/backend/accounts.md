...
这些是已存在的关联用例，本次未执行。配置凭据轮换后旧管理员cookie仍不走数据库撤销、旧cookie自动补建Session、跨服务提交后失败等边界由当前源码得出，不冒称专门故障注入或真实浏览器/多进程部署验收通过。账号行为变更需分别验证数据库账号、配置管理员、已有cookie和全新登录，不能用单一身份的成功替代全部身份。
