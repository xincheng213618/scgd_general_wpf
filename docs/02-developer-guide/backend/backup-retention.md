...
管理控制台的“数据库备份”按 backups:manage 权限读取文件清单，并在具备 jobs:read 权限时结合 database_backup 最近执行和启停状态。已有文件不覆盖最近自动备份失败或停用的提示，也不等于恢复演练通过。清单读取失败显示未知；“查看备份”进入 `/admin/cache#database-backups`，自动备份执行记录可直达任务历史。
...
跨进程同时备份、创建期间其它清理job、磁盘故障的多阶段部分成功、清理后实际恢复及各种旧凭据组合，不能由现有静态核对或正常路径用例宣布全量通过；尤其不能把quick_check等同于业务恢复验收。
