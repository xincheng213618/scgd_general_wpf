...
status: "current"
summary: "插件市场Flask后端的文件制品、索引、认证与API边界，以及隔离存储的启动测试。"
aliases: ["插件市场后端","上传401","Flask","marketplace.db","api/ready"]
code_paths: ["Web/Backend/app.py","Web/Backend/app_setup.py","Web/Backend/routes","Web/Backend/services"]
test_paths: ["Web/Backend/test_app.py","Web/Backend/test_app_releases.py","Web/Backend/test_upload_services.py"]
related: ["delivery.scripts","plugins.index"]
---
...
下面分为联网安装依赖和本地启动两个动作。启动会创建/修改指定存储并监听端口，不是只读诊断；不要直接复用生产存储测试。先确认授权和端口可用。

```powershell
Push-Location Web\Backend
python -m pip install -r requirements.txt
$testStorage = Join-Path $env:TEMP 'ColorVisionBackend-Local'
python app.py --storage $testStorage
Pop-Location
```
...
首次部署或大量手工改文件后，可在确认目标存储后运行 `python app.py --storage <目标目录> --refresh-all-indexes`；该操作会改写索引，不能作为默认只读排查步骤。
