...
网页附件、`FetchUrl` 和搜索深读共用 `CopilotWebPageToolSupport`。先下载并提取静态 HTML 或结构化资源；HTML 正文为空、内容稀疏或短正文包含脚本时，自动调用 `CopilotWebPageBrowserRenderer`，使用现有 WebView2 Runtime 执行页面 JavaScript 后提取可见正文、标题和同源链接。普通静态网页与 JSON 等结构化内容不启动浏览器。结果标明 `static HTTP content` 或 `browser-rendered DOM`，不会把未渲染的空页面当成已成功访问。
...
搜索查询不会改写草稿或触发模型历史计量，弹层期间的模型／消息刷新仍以实际草稿计量，详见[输入时的上下文计量](./copilot-view-model-architecture.md#输入时的上下文计量)。搜索匹配数或范围变化会同时更新提交按钮提示，零结果提示修改关键词；恢复匹配后提示将选中请求放回输入框。
...
会话查找的后台刷新继续更新匹配数与高亮；当前匹配对象未变时，不重复触发消息导航，流式正文、推理或执行内容更新不会把用户拉回旧匹配。用户改变查询、打开查找、选择上／下一项或明确执行 `/turn` 仍可主动定位同一消息。面板延期到布局完成后执行导航时，只接受原 ViewModel、原消息集合内的最新请求；查找导航还要求查找仍打开且目标仍是当前匹配。关闭查找、切换会话或卸载面板后的旧请求失效。`CopilotConversationFindNavigationTests` 通过真实 WPF 消息容器的导航事件验证这些边界；查找本身仍扫描相关消息，未缓存另一份历史内容。

自动跟随底部与主动消息定位共享一个 Dispatcher 操作及导航版本。查找、`/turn`、新发出的用户消息、会话切换和“回到最新消息”会替换旧导航；流式更新只合并自动跟随，不抢占等待中或执行中的主动定位。向上离开底部时取消尚未执行的滚底，即使本次布局同时增高；关闭查找只取消对应的查找操作，不能取消后来发出的普通定位。旧回调的收尾也不能清除新会话的操作。滚底只调用 `ScrollViewer.ScrollToEnd`，由虚拟化面板呈现末尾内容；同时对末行调用 `ScrollIntoView` 会形成行定位，在末行高于视口时可能把位置覆盖到行首。查找与 `/turn` 继续使用消息定位。

排版可能晚于正文通知改变消息高度。有效旧视口内的内容增高，且未观察到向上位移时，面板按增高前距底部的位置决定继续跟随；正在阅读历史的位置保留。初始零尺寸布局不作为“原本在底部”的证据。`CopilotChatPanelScrollLifetimeTests` 用真实 WPF 滚动位置覆盖导航竞争、卸载和 ViewModel 替换，并对照像素虚拟化、循环利用消息容器及延迟行增高。延迟增高测试控制行几何，不替代 Markdown 渲染或实机视觉验收。
...
回答中的本地文件链接支持当前工作区内的绝对路径、相对路径和 `file:` URI，以及 `:行:列` / `#L行C列` 定位。路径含空格时使用 Markdown 的 `<路径>` 形式；零值和溢出的行列号不会退化成普通文件链接。点击时重新检查当前工作区和文件存在性，再复用应用的资源打开路由；只向路径身份一致的活动文本编辑器发送定位，查找包含浮动停靠文档。独立查看器打开成功但不支持定位时保留原编辑器的光标，并提示定位未完成；没有行号的链接仍按普通文件打开。打开失败保留路由返回的具体原因，随后重试成功会清除旧错误提示。`CopilotLocalFileLinkNavigationTests` 在隔离工作区中覆盖实际 Markdown 超链接点击、真实 AvalonEdit 光标、浮动文档、中文空格路径和工作区切换后的复核，不调用系统外部查看器或改写文件。
...
工具活动记录区分“调用成功”和“结果完整”：分页、扫描受限、部分文件失败或读取截断时，折叠标题用警示色标出“结果不完整”，展开后显示限制原因与继续读取方向；同组多次调用也保留提示，真实失败仍优先显示失败状态。标题随窄窗口换行，悬停可读取完整的活动说明。提示随会话保存和恢复，旧活动记录不凭摘要文本补造状态。范围提示来自工具结果契约，不能据此宣称整个任务已完成。`CopilotPartialEvidencePresentationTests` 使用实际 WPF 活动模板验证单条／分组的颜色、展开内容和窄宽布局，并可输出离屏渲染供检查；它不等同于运行中桌面应用的人工交互验收。

公式由 `CopilotMarkdownMath` 识别、`CopilotMarkdownView.SpecialContent` 使用 WpfMath 渲染：行内支持 `$...$` 与 `\(...\)`，独立/多行块支持 `$$...$$` 与 `\[...\]`。行内代码跳过行内公式识别，解析/渲染失败回退文本；块公式经过 trim/拼接，不保证逐字节还原。`CopilotMarkdownView.xaml.cs` 把围栏代码优先于公式和表格解析。代码围栏接受零至三个前导空格以及至少三个同类反引号或波浪号；结束围栏必须使用相同字符、长度不短于开头，且之后只能有空白。反引号围栏的信息串包含反引号时不作为开头。代码内容按开头缩进去除至多相同列数，其余空白、结尾空行及围栏内 Markdown/公式原样进入代码卡和复制载荷；空块和未闭合块也保留为代码卡。围栏外公式仍按原路径渲染。公式显示不是模型执行数学验证的证据。
...
元数据中的测试覆盖命令展示/执行分离、composer 状态、分页、mentions_v2 门控、部分 ViewModel 交互及桌宠优先级。`CopilotMarkdownViewTests` 在无窗口 STA 线程检查文档树中的代码正文、复制载荷与围栏后的段落／公式，覆盖不同围栏字符和长度、开头缩进、空块、未闭合块、尾部空行以及两种块公式分隔符。`CopilotMarkdownTableBoundaryTests` 检查末尾转义竖线、实际外边界和显式空单元格。测试路径存在不代表本次已运行；公式实际字体布局、系统剪贴板/保存窗口、真实焦点和桌宠确认窗口仍需要对应 WPF 验证。修改命令时同时核对 Catalog、AvailabilityPolicy、ViewModel handler 和 Panel 键盘路由，不只更新一处帮助文案。
