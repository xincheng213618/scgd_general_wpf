...
| `MaximumBadPixelCandidates` | 1000 | 0..100000；总数不截断，Table/Geometry/Overlay 截断 |
...
- min/max/mean 和总体标准差只使用有限值；NaN、正/负 Infinity 分别计数。percentile 使用 `(n-1)` rank 的线性插值。
...
- 坏点候选使用同 ROI、同通道的局部中位数与 `1.4826 × MAD`；同时满足 Sigma 阈值和标称范围最小偏差才返回，边界邻居不足 3 个时跳过。
...
失败使用 `roi_required`、`roi_empty_after_clip`、Runner 的 `unsupported_format` 或参数校验 failure；取消返回 `Cancelled`。调用方始终释放整个 `AlgorithmResult`。
...
`BatchAlgorithmAnalysisProcessor` 使用同一 Invocation/Runner，逐文件输出 JSON 或 CSV bundle；它不复用图像输出格式策略。`LocalFlowImageAlgorithmAdapter` 可对进程内 RAW 帧执行同一分析且不取得外层 frame lease 所有权。旧 MQTT/设备 `AlgorithmNode` 未改动。

M1 没有加入 Copilot 白名单：当前 Copilot 图像工具没有受审批 ROI 输入契约，Catalog 的可发现性不能绕过审批、目录、覆盖和数量限制。

## 性能与复现

基础扫描是 `O(ROI 像素数 × 通道数)`。整数图每通道使用 256 或 65536 个精确计数；32F 为精确 percentile 保存并排序有限值，因此峰值内存为 `O(有限 ROI 样本数 × 通道数)`。坏点检测另做一次邻域扫描，适合可控 ROI；大图可关闭该参数。执行每 8/32 行检查取消并报告进度，不做隐藏降采样。
