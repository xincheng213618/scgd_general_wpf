...
输入框使用一个有界的本地 Slash 命令目录。输入 `/` 会显示全部候选，继续输入会按命令名前缀过滤；Tab 补全第一项，Enter 补全后直接执行，也可以点击任意候选。固定目录复用既有能力：`/status`、`/doctor`、`/tasks`、`/usage`、`/stats`、`/context`、`/permissions`、`/hooks`、`/skills`、`/mcp` 提供本地状态与诊断，`/diff`、`/compact`、`/review`、`/plan`、`/goal` 提供工作区和任务控制，`/resume`、`/rename`、`/new`、`/fork`（别名 `/branch`）提供会话导航与命名，`/copy`、`/export` 提供可见会话内容出口，`/model`、`/reasoning`（别名 `/effort`）提供模型入口，`/btw` 提供旁路提问。Skill 还可以按 `$name` 或 `/name` 出现在同一补全目录中；固定候选和动态 Skill 候选合计最多 30 项，确保当前 27 个固定命令在裸 `/` 查询中可见，并继续为动态 Skill 保留至少 3 个候选槽位。`CopilotComposerReferenceCatalog` 为输入框提供独立的 `@` 目录；既可直接键入，也可从 `+` 菜单在当前光标或选区位置插入，已有未闭合 mention 不会重复追加。它从 `TemplateControl.ITemplateNames` 的模板类型与已保存模板名、`CopilotMenuToolSupport` 和当前 solution root 的有界文件索引生成最多 12 个候选；空查询在模板、菜单和文件之间均衡取样，输入查询后按标题、路径和代码相关性排序。模板和菜单候选先在 UI 线程立即显示；文件扫描在后台最多索引 5,000 个受支持文件，跳过 `.git`、IDE 状态、依赖和构建输出目录，同一次未闭合 `@` 查询复用结果，每次新的 `@` 查询重新取样，因此刚创建的文件不会被长期缓存隐藏。仅等待文件结果或最终没有命中时，`@` 弹层分别显示“正在索引”和“未找到”状态；索引完成前 Enter 不会误发未完成查询，Tab 也不会误触发排队。选择文件会走既有附件边界；选择模板或菜单会生成带稳定 source ID 的 `CopilotContextItem`，保存模板引用只携带身份元数据，菜单上下文明确不构成执行请求。完成后的文本使用 `@[标题]` 闭合标记，防止继续被当作未完成查询。
...
`/usage` 从完成回答上的 `ReportedUsage*` 持久化字段聚合当前 conversation 的 Provider Token 元数据；`CopilotHostedTurnCompletion` 只在成功完成时同时提交消息级用量和兼容的 `LastUsage`，取消、失败或未返回用量时不会制造估算值。加载旧状态时，`EnsureValid` 只把既有 `LastUsage` 迁移到最近一条未中断且尚无消息级用量的 assistant 消息；应用退出时恢复为中断状态的新回答不会错误继承上一轮用量，可能只含部分执行量的 Agent budget 也不会被当作成功完成量。分支通过既有消息快照自然保留分叉点以前的用量，不继承分叉点以后的回答。聚合采用饱和加法，保留 Provider 报告的 total 与 cached input，并明确不代表账户账单、套餐额度、价格或 rate limit。该边界对齐 [Codex `/usage`](https://learn.chatgpt.com/docs/developer-commands?surface=cli#view-account-usage-with-usage)、[Claude Code `/usage`](https://code.claude.com/docs/en/costs) 的可观察性，同时采用 grok pager `/usage` 对未知账单边界不自行推断的做法。
`/stats [7|30|all]` 在 `/usage` 的消息级证据之上聚合全部本地 conversation，默认显示最近 7 天，也可选择最近 30 天或全部历史。它按本机日历日汇总活跃会话、用户提问、完成/进行中回答、已报告与未报告回答、输入/输出/缓存 Token，并从用户提问计算当前与最长连续活跃天数；日报始终有界为最近 7 个日历日或最近 7 个活跃日。会话分支会优先使用 `BranchOrigin.ParentConversationId` 与 `ThroughMessageId` 跳过复制前缀；源会话缺失时以 `ForkedAtUtc` 和保留的消息时间戳做安全回退，避免同一历史 Token 因分支快照重复累计。命令不读取 Provider 账户、价格表或远端额度，也不把未报告的失败调用猜成费用；它对应 [Codex `/usage` 日/周/累计活动](https://learn.chatgpt.com/docs/developer-commands?surface=cli#view-account-usage-with-usage) 和 [Claude Code `/usage` Stats 视图](https://code.claude.com/docs/en/commands) 的历史可观察方向，但只报告 ColorVision 当前能够从本地状态证明的数据。
`/tasks` 从 `CopilotAgentTaskHost.ScheduledRuns` 捕获当前活动运行与有序队列，再从 `CopilotAgentTaskIndex` 补入暂停、受阻、等待用户或可恢复的历史任务；已经重新进入 host 的 conversation 会从待处理区排除，避免同一任务被显示两次。快照只复用 UI 已显示的规范化会话标题，并补充结构化模式、状态、等待或运行时长、checkpoint 就绪标记、剩余项和可恢复标记；它不展开提示正文、blocker 自由文本、工具参数、内部 ID 或 checkpoint 内容，待处理区最多显示 20 条并报告省略数量。命令在活动请求中可用，但只读取既有状态，不调用模型或工具，也不执行暂停、取消、重排或恢复。该入口对应 [Codex `/agent`](https://learn.chatgpt.com/docs/developer-commands?surface=cli#switch-agent-threads-with-agent) 的活动线程可观察性，并采用 [Claude Code `/tasks`](https://code.claude.com/docs/en/commands) 与 grok pager `/tasks` 的后台任务快照边界；实际切换、继续和放弃仍留在现有 WPF“Agent 任务”列表。
