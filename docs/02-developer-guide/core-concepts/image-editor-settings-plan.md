...
显示滤镜保留 `ImagePresentation.SceneEffect` 的完整画布作用范围，图像与叠加内容一起受影响；它不提交源像素，也不保证后台 source 输出包含该效果。图像/叠加承载与输出限制见[ImageEditor](../../04-api-reference/ui-components/ColorVision.ImageEditor.md)。
