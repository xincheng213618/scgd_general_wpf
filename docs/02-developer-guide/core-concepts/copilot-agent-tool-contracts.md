...
用户在活动任务中提交且被接收的新补充输入，会允许本轮已有工具面中的成功本地证据调用重新执行一次，包括 `ReadLocalFile`、`ReadAttachedFile`、`GrepText`、`SearchFiles` 和 `ListDirectory`。该次新观察之后，相同调用继续去重，直到又有新的用户输入；写调用、失败调用、尚未结束的调用和相同 Provider CallId 不因此重放，文件范围、审批与全局调用预算也不扩大。外部文件变化本身不自动解除去重，用户可在原任务中补充“文件已更新，请重新读取”。已成功应用且确认文件内容或存在状态变化的工作区补丁，会单独解除上述五类工具中受影响的成功调用去重，供保存后核验。文件读取按已读取的精确路径匹配，搜索和目录列表按实际查询范围匹配，即使先前没有任何匹配结果也能在新增、修改或删除后刷新；未指定路径时，搜索可覆盖多个根，而目录列表只对应实际列出的目录。该范围只用于本轮证据失效，不增加文件访问授权；无变化的补丁、写入前校验失败以及不相干目录不会解除去重，运行中调用仍不可重复启动。执行器释放读锁到 bridge 登记结果之间也属于未结束窗口：若此时先登记了工作区变更，待旧读取结束后按其真实文件或查询范围决定是否解除去重，避免迟到的旧结果阻止保存后核验；不相干范围、失败读取和全局调用预算保持原约束。多文件补丁开始写入后失败，会对已尝试写入或恢复的精确路径单独标记需重新核查，允许上述本地观察再执行一次；该标记不是文件变化或恢复完成的证据，也不解除失败写调用的去重。`CopilotWorkspaceReadInvalidationTests` 通过正式 Runtime 的输入准入与工具循环检查原值／新值，并覆盖拒绝的补充、五类本地观察、运行中去重、失败与写调用保持及预算上限。
