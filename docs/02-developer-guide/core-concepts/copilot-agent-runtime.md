...
2. 将运行宿主从 `CopilotChatViewModel` 抽成进程级 task host，使任务在切换会话后能够继续执行，并为完成、阻塞和审批等待增加跨会话通知；任务索引继续作为单一 UI 查询入口。
3. 为长任务增加用户可配置的请求 Token、工具轮次、Agent pass 和总时长预算，并保留 profile 默认值与单次覆盖的清晰优先级。
