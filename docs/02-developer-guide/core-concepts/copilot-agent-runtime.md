...
1. 继续按 `InspectTcpPort`、`InspectWindowsProcesses` 的模式增加固定参数、固定实现的常用只读诊断工具；通用 Shell 在没有系统沙箱时仍保持每次原生审批。当前 Shell runner 已完成 Job Object 进程树归组；后续再增加受限 Windows 身份、文件系统与网络边界，形成真正的系统沙箱。
