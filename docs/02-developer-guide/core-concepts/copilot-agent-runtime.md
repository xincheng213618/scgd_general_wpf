...
`Auto` 模式不会因为当前解决方案存在搜索根目录就自动暴露搜索工具。普通概念问答直接由模型回答；只有用户明确指向当前项目/代码、公开网页/最新信息或给出 URL 时，才分别开放本地搜索、`WebSearch` 或 `FetchUrl`。数据库、日志和通用 Shell 同样按当前请求意图开放，避免较弱模型把无关的系统问题误路由成 SQL 或日志诊断。外部 MCP 中名称或描述可识别为文件搜索、网页搜索和 URL 读取的工具也服从同一意图门槛，其他设备、状态与业务工具仍按各自 `CanHandle` 判断。

短追问不会让会话刚用过的能力凭空消失：最近一轮真正成功执行过、只读、幂等且无需审批的工具可以在 24 小时内随同 checkpoint 安全续租；写工具和通用 Shell 不续租。对“现在呢”“再检查一遍”等刷新式追问，单端口诊断还会从最近可见会话或 checkpoint memory 恢复端口号，并重新触发真实检查执行契约。新主题、长请求或明确本地代码任务仍重新计算工具面。
