...
2. 按高频业务问题继续增加参数化语义能力，使常见查询不依赖模型理解表结构；通用 SQL 继续保持单语句、原生审批、结果有界与敏感信息脱敏。
