...
主 Agent 还可自主调用 `DelegateExplore`，把范围较广或预计产生大量中间证据的多文件调查交给全新的只读 Harness Session。模型可在同一个响应中发出最多两个互不依赖的调用；Framework 函数层并发执行，工具自身再用两个可取消的槽限制子运行，第三个调用等待槽位。不同任务使用不同只读资源键，重复任务继续服从现有同资源互斥与 no-progress 保护。

子 Agent 不继承父会话历史、附件、checkpoint、todo、可写根、外部 MCP 或审批状态，只接收自包含调查任务、最多四个搜索根、活动文档和仍处于这些根内的项目指令；存在活动文档时优先保留其所在根。其工具面固定为上述四个工作区读取能力，不能调用 Shell、数据库、网页、写工具或再次委派。每次子运行最多 8 次工具调用、2 个 Agent pass、90 秒、16,384 个请求 Token 和 12,000 字符返回内容；更小的父运行预算会同步收紧工具次数、pass、时长和 Token。

同一父请求的全部 Explore 运行共享委派 Token 池：总量通常为父请求预算的一半，最低 4,096、最高 32,768 Token；单个子运行按并发公平分配并至少需要 4,096 Token。成功后按真实用量结算，异常或取消时保守消耗已预留额度，预算不足时不再启动新的供应商调用。每个子运行生成独立 `explore-*` ID，父工具 trace 的 `CallId` 与该 ID、停止原因、排队时间、工具次数和 Token 预算一起持久化；子运行内部的逐工具噪声仍不复制到主聊天。供应商调用数、Token 与估算用量继续归集进父运行预算，父 Agent 收到结果后仍需综合证据并完成最终回答。这一角色分工保持了 [OpenCode Agents](https://opencode.ai/docs/agents/) 中“主 Agent 选择专用只读 Explore、子 Agent 使用独立权限边界”的核心语义，同时保留 ColorVision 自己的 Framework 预算和审批模型。
...
1. 在固定只读 `Explore` 角色之外增加可配置角色目录和独立的外部文档 `Scout`；任何写操作仍只由父 Agent 通过现有预览和原生审批路径执行。
