...
为避免项目 Skill 持续增长后放大每次模型调用的 L1 元数据成本，运行时在 Framework 完成一次文件发现后对候选项去重，并根据当前请求与 `name`、`description` 的中英文相关性选择最多 16 个活跃 Skill，同时限制本轮公开给模型的 Skill 名称和说明合计不超过 8,000 字符；单个超长但最相关的 Skill 至少可以进入，用户显式点名的 Skill 具有最高优先级，同一运行的发现结果会被缓存。运行诊断会显示候选数、启用数、预算裁剪数，以及真正被 `load_skill` 或参考资料读取路径加载的 Skill 名称。

运行结束后，宿主只把“本轮被选择”和“本轮实际加载”两个计数写入本地有界状态 `Copilot/State/skill-usage.json`。状态最多保留 128 个名称，单文件上限 1 MiB，损坏或超限时安全重建；不保存提示正文、Skill 内容或用户问题。连续至少 20 次被选择且从未加载的 Skill 会自动降级为 explicit-only，不再占用后续请求的默认 Skill 元数据预算；用户在当前问题中以 `$skill-name` 或完整 Skill 名称直接点名时仍可加载。一次真实加载会解除历史低使用率降级，使该 Skill 重新参与隐式匹配。设置诊断页显示选择次数、加载次数、加载率、最后选择时间和当前历史降级状态。
...
1. 通用 Shell 在没有系统沙箱时继续保持每次原生审批。当前 runner 已完成 Job Object 进程树归组；后续再增加受限 Windows 身份、文件系统与网络边界，形成真正的系统沙箱。
