...
明确的工作区修改请求使用同一类执行契约：只有 `ApplyWorkspacePatch` 成功完成才算执行了修改，模型仅输出代码或声称“已经修改”不能满足契约。修改采用 `PreviewWorkspacePatch -> ApplyWorkspacePatch` 两阶段协议；预览只允许对现有文本文件做一次精确且唯一的 `oldText/newText` 替换，并绑定当前文件 SHA-256。应用阶段必须使用原预览 ID、重新核对可写范围和 SHA-256，并经 Agent Framework 原生审批后原子替换文件。预览后文件发生变化时拒绝覆盖。成功应用的预览可由 `RollbackWorkspacePatch` 在再次审批且当前文件仍匹配应用后 SHA-256 时恢复原始字节。预览只在当前进程中保留 30 分钟，不跨应用重启构成恢复授权。
...
- 工作区文本修改拆成只读的 `PreviewWorkspacePatch`、高风险非幂等的 `ApplyWorkspacePatch` 和 `RollbackWorkspacePatch`。默认可写根只包含当前解决方案目录；用户显式交给 Copilot 的现有文本文件和当前活动文档可作为精确文件授权，普通附件不会自动扩大可写范围。路径穿越、重解析点、非文本扩展、超过 1 MB 的文件和多重匹配均被拒绝。
