...
`CopilotAgentTaskHost` 是进程级单活动运行宿主，运行通过稳定 run ID 绑定原会话，并集中持有 cancellation token、类型化 run control、checkpoint-ready 边界和 completion task。第二个运行在活动槽释放前会被拒绝；暂停只能在 checkpoint 边界后发生，明确取消可以覆盖待处理的暂停，订阅者异常不会破坏运行或占住活动槽。Agent 执行期间允许切换或新建会话，事件和最终结果仍写回启动运行的原消息；普通 Chat 暂时保持会话切换锁定，因为其历史构造仍依赖当前选中会话。查看其他会话时，标题区只显示一条可点击的“正在运行/正在暂停”状态；任务结束后原位更新为完成、暂停或阻塞结果，不展开诊断明细。
...
2. 为长任务增加用户可配置的请求 Token、工具轮次、Agent pass 和总时长预算，并保留 profile 默认值与单次覆盖的清晰优先级。
3. 在单活动运行协议稳定后，再增加有界多任务队列；写任务仍必须串行，并继续复用 capability resource、审批和审计约束，而不是简单并发多个 ViewModel 请求。
