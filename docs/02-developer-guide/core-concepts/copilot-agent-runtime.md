...
- 设置页 `Refresh Discovery` 强制绕过缓存。实时发现会比较有界能力签名、递增服务能力 revision 并发出进程内变更事件；健康快照标明本轮使用 live 还是 cached discovery。
...
1. 订阅 MCP `notifications/tools/list_changed`，在服务主动声明工具变化时提前失效缓存；TTL 和手动刷新继续作为兜底。
