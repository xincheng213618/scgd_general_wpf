...
1. 将同一 Capability Fabric 扩展到 LAN 和 ServiceHost，保持能力白名单、身份、审批和审计语义一致。
2. 为长任务增加结构化 artifact/checkpoint 引用，让重新规划可以恢复已验证证据而不依赖模型上下文重放。
