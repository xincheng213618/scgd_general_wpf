...
1. 将 Catalog revision 写入 AgentSession checkpoint；恢复任务时检测 capability drift，失效已不存在的计划步骤并要求 Framework 重新规划，避免恢复后调用过期能力。
2. 将同一 Capability Fabric 逐步扩展到 LAN 和 ServiceHost，保持能力白名单、身份、审批和审计语义一致。
