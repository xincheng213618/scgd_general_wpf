...
普通 follow-up 入队时捕获 Profile、运行配置、附件、Skill/审查目标和项目上下文；实际执行前才重新捕获该会话历史，以包含前一轮已经完成的消息。耐久恢复记录拥有入队时的活动文档、解决方案根、附加只读根和 Live Context 快照；重启时当前解决方案不匹配或原路径失效就恢复草稿，不借用启动后的当前上下文自动执行。Steering 是发给运行中任务的消息，不是新建 turn；取消、重启恢复和排队本地命令规则见[会话与工具](./copilot-agent-session-and-tools.md)。
