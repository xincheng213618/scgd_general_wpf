---
knowledge_id: "copilot.extensions"
...
业务菜单可以显式附加一次快照：流程右键“问 AI 分析当前流程”由 `FlowEngineManager.AskCopilotFlowCommand` 调用 `FlowCopilotService.AskAboutCurrentFlow()`，附带当前流程、选中或活动节点、节点参数及最近运行/失败证据；设备右键“问 Copilot”由 `DeviceService.AskCopilotCommand` 采集该设备的诊断上下文。两者经 `CopilotBusinessContextCoordinator.DispatchDiagnosis` 发起诊断请求，附加上下文本身不执行流程或控制设备。

图像右键“问 AI 分析当前图像”由 `ZoomEditorToolContextMenu` 采集 `ImageViewConfig` 属性、ROI/选区和标注摘要，再由 `CopilotBusinessContextBuilder.BuildImageContextItem` 构造结构化文本。该入口不附加图像像素，不能据此声称已观察像素或完成图像算法检测；这也不限制[运行时的显式图片附件](./copilot-agent-runtime.md)。

`EditTemplateJson` 的 Copilot 操作附加编辑器模式、模板/Schema 身份、JSON 语法状态和相对加载快照的修改状态。`BuildCopilotSnapshotContextItem()` 只保留当前 JSON 的前 16,000 字符并标记截断，不保证完整参数可供补丁计算。`JSON validation` 由 `JsonHelper.IsValidJson` 得出，不是 Schema 或业务参数验证；`Unsaved changes` 仅比较当前 JSON 与加载参数时 `_loadedJsonSnapshot` 的 `Trim()` 后文本，不查询数据库确认是否已保存。补丁的来源匹配和不自动保存边界见[模板 JSON 预览与应用](./copilot-agent-tool-contracts.md#模板-json-预览与应用)。本页已有 MCP/Hook 测试不证明这些菜单、WPF 编辑器快照或图像上下文的端到端行为。
...
- 默认 `approval` 将整个服务视为高风险、可写、非幂等能力，每次精确调用都经过 `ApprovalRequiredAIFunction` 的原生审批边界；人工或自动复核的决定规则见[原生审批契约](./copilot-agent-tool-contracts.md#原生审批与参数快照)，配置服务不构成调用批准。
...
- 进入运行时的 MCP JSON Schema 会传给模型；本地通过共享 `CopilotToolInputContractValidator` 递归验证支持的嵌套对象、数组及字段约束，并限制参数总长度。声明不支持约束的工具不会暴露；本地输入校验失败时不发送 `tools/call`。远端仍负责自己的权限与业务校验，本地 Schema 通过不证明远端操作可执行。共享规则见[输入校验与无效调用](./copilot-agent-tool-contracts.md#输入校验与无效调用)。
- 工具发现会记录进程内健康快照，包括连接状态、发现数量和实际暴露数量；设置页只显示紧凑摘要，不把连接错误铺到主聊天区。
- 单个服务不可用只产生运行时诊断，不会终止 Agent 请求；普通聊天界面隐藏这类基础设施噪声。

写入或非幂等 MCP 调用进入 SDK 后，如果因传输错误、取消、响应损坏或结果解析失败而未取得可用的协议回复，适配器返回 `OutcomeUnknown` / `tool_outcome_unknown`，不能把本地请求结束理解为远端没有执行。结果只包含固定说明，不回显异常正文；模型输出与工具事件保留同一失败码，后续 checkpoint 禁止直接恢复工具任务，需重新规划并核对外部状态；无工具的 Finalize 仍可整理已有结果，详见[显式有界重试](./copilot-agent-session-and-tools.md#显式有界重试)。调用前已取消、未批准或本地参数无效时不会发送 `tools/call`；正常返回的 `CallToolResult`（含 `isError`）和有效 JSON-RPC 错误回复仍保留原有处理，不因失败本身被改判为未知。只读幂等工具也保留既有重试规则。`CopilotExternalMcpToolOutcomeTests` 通过正式 provider、SDK 与受控 HTTP 覆盖远端模拟写入后的丢包、坏 JSON、取消，以及审批、嵌套参数和明确拒绝对照；不调用真实远程设备。

设置中的每行由名称、Streamable HTTP 地址、token 环境变量名、默认策略和工具白名单五段组成。下面只是配置格式示例，保存/发现/实际工具调用仍是分别授权的动作；不要把凭据值填进第三段：

```text
lab | https://mcp.example.com/mcp | LAB_MCP_TOKEN | approval | get_status=read-only,apply_patch=approval
```

官方 SDK 和传输说明见 [Model Context Protocol C# SDK](https://github.com/modelcontextprotocol/csharp-sdk)。入站 ColorVision MCP server 是另一条会话与确认链，见[Local MCP](./colorvision-mcp.md)，不要将外部 client 配置和本机 server token 混用。
