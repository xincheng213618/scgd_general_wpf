...
九点 RGB 分离与单十字复用 FindCross 模板类别和数据库类型 `63`，但结果格式不同：单十字沿用 `1.0` 的 `result[]`；`LocalRgbCrossNode` 写入 `2.0` 主记录及 `colorvision.rgb-cross-measurement` JSON。九点参数放在同一模板的 `rgbCross` 段，不把旧阈值/Hough 参数隐式映射过去。ARVRPro 选择“九点十字 RGB 分离”后读取此格式；原光学中心解析跳过九点记录。独立 ImageView 九点执行不入库，关闭结果窗保留叠图。完整九点输出和节点说明见显示计量主题。

“本地十字定位”位于 `Flow_CustomNodes`，节点标识 `LocalFindCross`。设置“图像文件”“算法参数(JSON)”“搜索区域”“搜索区域关注点”和“结果目录”即可定义输入和保存位置。
