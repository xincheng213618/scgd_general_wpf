...
当前 `ServiceHostManagerWindow` 提供上述维护动作；`MenuRegisterThumbnail` / `MenuUnregisterThumbnail` 也调用同一客户端，但菜单声明为 `Visibility.Collapsed`，不能描述成默认可见菜单。ServiceHost 将命令映射到自身 `Tasks` 中的固定脚本，并要求 broker ticket；客户端 UI 不需要自行启动源码目录的管理员脚本。`RegisterFileAssociations.ps1` 也是写入两种 CLSID 和扩展名绑定的来源，还会改打开命令、图标及其它文件关联，不是仅清理缩略图的等价入口。
