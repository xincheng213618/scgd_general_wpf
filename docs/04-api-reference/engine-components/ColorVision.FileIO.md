...
“全局选项 → 常规 → 文件归档 → 启用 CVRAW 文件缓存”与“本地缓存管理 → 图像文件缓存”共用 `CvRawFileCacheConfig.IsEnabled`，通过 `[ConfigSetting]` 注册，默认开启并持久保存，启动时应用到 `CVFileReadCache.IsEnabled`。关闭后立即停止命中及填充槽位，保存图片和更新校正/POI 参数仍正常写入磁盘；在途读取继续使用原缓冲，最后一个读取归还后释放槽位。重新开启从下一次读写开始缓存；开关不改变校正缓存、CVCIE、实时预览或每视图 RAW/显示缓冲复用。对比的是应用文件缓存效果，关闭不等于清空 Windows 文件缓存。
...
同一打开器的 RAW 加载串行，旧请求不能覆盖新图；下一次填充前先停止使用上一幅图的测量源，POI 和原图层借用期间受到同步保护。带色度 JSON 的 RAW 测量直接借用该输入数组，不复制另一份 RAW；重新打开会重新读取参数。清空/销毁视图会放开输入数组引用，等待中的读取先结束，避免释放正在读写的内存。文件缓存流在 UI 更新前归还，避免保存/释放槽位与 UI 调度互相等待。

`LocalFrameFileService` 的文件回退共用该文件槽位，本地校正和实时 POI 仍优先使用已有流程帧。按需 XYZ 测量继续使用原来的只读映射，不新增全幅 XYZ 缓存；CVCIE 文件本身不占 CVRAW 槽位。“本地缓存管理”的两个 Tab 分别查看校正缓存和图像文件缓存，“释放全部”统一释放这两类进程内缓存并保留磁盘文件，当前显示/测量所需的独立图像内存不属于文件槽位。此功能保持同步保存，旧服务仍可读取已完成的文件，不提供异步保存完成前的路径承诺。
...
- `CvRawPixelBufferTests`：路径/JSON 改变时复用输入、相同字节数但不同布局时替换、坏 payload 拒绝覆盖、8/16 位单色/BGR 与 8 位 BGRA 显示一致、冻结位图替换、过期请求/清空、色度参数重载与 POI 借用同步。16 位四通道仅验证输入缓冲，编辑器当前的 `ImageSourceMetadata` 不接受 Rgba64；本优化不扩展显示格式。使用同一真实样本环境变量可记录连续读取的托管分配和耗时；这些结果不证明其它处理链零分配或现场不会产生内存碎片。
