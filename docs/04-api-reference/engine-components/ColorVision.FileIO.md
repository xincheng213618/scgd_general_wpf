...
`LumFourColorCorrectionCalculator` 提供 MATLAB 单点、MATLAB RGBW 与 Python RGB 三种计算模式，均使用 `CVRawManualCieConfig` 承载九个结果系数。输入统一为相机侧与光谱参考侧的 `Y/CIE x/CIE y`；`Yxy → XYZ` 使用原始有限数值，只要求作为分母的 CIE y 不为 0，不裁剪负 Y、负色度、中间反解通道或最终矩阵系数。Python 模式输出的是 XYZ→XYZ 变换，不能与另外两种模式输出的完整校正系数混用。

`LumFourColorCalibrationFile` 在修正入口识别两种 JSON 原文件，文件结构不自动选择计算模式；单点 / RGBW 的另存保持以下结构，Python RGB 的独立导出见后文：
...
- `CorrectPythonRgb` 复现 `calibration_pro/main.py` 的 `run_fr_adv`：以 R/G/B 的 XYZ 为列，计算 `M_reference × inverse(M_camera)`。实现用转置线性方程求解各行，不显式求逆；保留原 Python 的 `abs(det(M_camera)) < 1e-12` 拒绝条件，零 CIE y、非有限数值、溢出仍拒绝。R/G/B 各自的参考亮度都参与，W 与原矩阵不参与。
- `LumFourColorCalibrationSession.SetMode` 使用 `LumFourColorCorrectionMode`，分别建立一、四、三组数据；两个窗口均显式选择模式，切换时清除测量数据与旧结果。Python RGB 不要求 W，仍检查三组的完整性与重复光谱结果 ID；从采集窗口打开手工窗口时带入当前模式。
- 单点 / RGBW 的 `LumFourColorSourceSnapshot.SaveCopy` 使用原文档的 `SerializeCorrection` 按原格式另存；Python RGB 则通过 `SerializeCalibrationFile` 导出标准 `a…i` JSON，增益全部为 1、曝光全部为 0，与 Python 输出一致，不沿用源文件的 `Gain/pa`、位深和附加字段。按钮显示“导出 XYZ 矩阵”，默认文件名带 `_PythonRGB_XYZ`；原文件仍用于取图与来源核对，三种模式的导出都禁止覆盖原文件并复核内容指纹，不自动安装到校正模板。
- `LumFourColorCalibrationWorkflowWindow` 是校正文件管理与“应用与工具”的主入口。单点采集一组相机 POI 与光谱仪数据，RGBW 按相同过程完成四组；相机侧与光谱侧的采集顺序不限。侧栏显示相机、光谱各一行 Y/x/y 六个输入框；图像来源、POI、曝光、XYZ、光谱结果 ID 与完整相对光谱放入默认折叠的“测量详情”。
...
- `LumFourColorDataChecks` 在采集侧检查 Yxy、峰值 AD 与波长顺序，IP 合格范围为 30%～95%，不裁剪有限负值。样本变更事件统一撤销旧计算；重采失败不能复用旧侧数据，未修改的光谱记录不得以同一结果 ID 分配到多个色块。文件来源未知或不一致、IP 缺失或范围异常可由操作员在默认取消的集中提示中明确继续；这不是自动质量认证。历史选择、状态失效与曝光边界见[四色校正采集](../../01-user-guide/devices/calibration.md#四色校正采集)。

`LumFourColorFileCompatibilityTests` 以实际执行外部 Python 单点与 RGB 函数得到的输出作为对照，验证两种原文件格式、Python XYZ 变换的三组参考匹配与独立导出，以及 RGBW 的参考色度与 W 亮度约束、原格式及附加字段往返、无效输入和保存保护。`LumFourColorWorkflowSafetyTests` 覆盖两种窗口的 Python RGB 三行门禁、Excel 粘贴、模式切换失效与独立导出标签。Python 对零 CIE y / 零反解通道的补值不沿用，本窗口继续拒绝不可计算输入。
