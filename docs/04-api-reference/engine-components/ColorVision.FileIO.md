...
- `LumFourColorCalibrationWorkflowWindow` 是校正文件管理与“应用与工具”的主入口。单点采集一组相机 POI 与光谱仪数据，RGBW 按相同过程完成四组；相机侧与光谱侧的采集顺序不限。界面显示 POI 的原始 XYZ 与 x/y，以及光谱 Y/x/y、结果 ID 和完整相对光谱。
- `LumFourColorPoiEditor` 复用 `ImageView` 的圆形、矩形绘图及尺寸面板，按图像中心坐标交给 `PoiMeasurementService.CalculateRaw`，保留原始负值。每个色块一个 POI；形状或位置改变立即撤销旧测量，绘制结束后重算，越界区域不能用于计算。预览区域不接受另外打开或拖入图片，以免显示图与测量帧脱离。
- 相机采集复用 `LocalCameraCaptureService`，未连接时按当前设备 Camera ID、测量模式和位深自动连接，选择的校正模板必须启用四色校正并生成 CIE，采集记录校正文件内容指纹。无设备环境可加载已有 `.cvcie` 验证 POI 和计算链；重新取图只清除该色块的 POI 与相机测量值，保留已采集的光谱。
...
- 采集窗口的 `ReferenceYInput` / `ReferenceCieXInput` / `ReferenceCieYInput` 支持直接录入及覆盖采集值。`IsReferenceEdited` 区分当前手动参考与原始光谱记录；每次输入先撤销有效参考和旧计算，再校验完整 Yxy。手动参考不要求波形，交给计算器的 `Spectrum` 为 null；原始波形和元数据仅保留供核对，`RestoreReference` 可恢复原始 Yxy。只有未修改的设备记录参与重复结果 ID 检查，手动值统一进入计算前的人工复核。
...
- `Test/ColorVision.UI.Tests/LumFourColorWorkflowSafetyTests.cs`：IP 端点与异常、原始负值和元数据保留、无效替换清除旧值、来源待核对、重复光谱 ID、文件指纹与副本保护，以及窗口输入变化 / 失败后的保存门禁；不替代真机采集、真实数据库历史选择或相机过曝检测。
