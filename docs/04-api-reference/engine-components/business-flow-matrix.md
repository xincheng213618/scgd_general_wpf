...
这页用于 Engine 交接、排障和改需求时快速定位“业务能力、代码入口、配置来源、验证方法”。它不是替代 [业务交接手册](./business-handoff.md)，而是把横向业务场景摊开：接手人员可以先按问题类型找到入口，再进入设备链路、模板链路或结果链路深挖。
...
2. 如果已经拿到具体需求或缺陷，进入 [Engine 业务场景交接手册](./business-scenario-playbook.md)，按步骤处理。
3. 再进入 [Engine 业务交接手册](./business-handoff.md)，理解完整执行链。
4. 改动完成或准备交付时，用 [Engine 变更影响与验收清单](./engine-change-impact-checklist.md) 收集证据。
...
| 修改 DLL 发布 | 对应 `.csproj`、`Directory.Build.props`、`Scripts/` | 文档配置里硬写版本 | [UI DLL 发布手册](../ui-components/publishing.md)、[UI DLL 发布矩阵](../ui-components/release-matrix.md) |
...
- [项目包交接手册](../projects/project-handoff.md)
