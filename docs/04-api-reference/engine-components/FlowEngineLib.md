...
标记 `Obsolete` 的节点类型会从 `STNodeTreeView` 的新建/右键目录和 Copilot 节点目录中排除，但仍由节点类型注册表保留，因此旧画布可以继续反序列化。当前包括 `/10 MQTT` 的 4 个旧节点、`/09 合规验证` 节点，以及已下线模板对应的旧校正/图像拼接节点。不要删除这些类型，除非已经完成存量流程迁移。
