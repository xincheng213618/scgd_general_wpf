...
- 根目录的 [Plugins/README.md](../../../../Plugins/README.md) 现在已经明确区分了“当前源码里真实存在的插件目录”和历史残留名称。
- API 参考中的本页则只保留 Pattern 相关能力在当前仓库中的剩余落点说明，不再把它写成现存标准插件。
- 如果两个入口存在表述差异，应优先以源码目录和运行时装载行为为准。
