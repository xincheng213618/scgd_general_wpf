...
Windows build 22000 或更高版本默认尝试将菜单与原生最小化、最大化和关闭按钮合并到同一行。`MainWindowConfig.UseCompactTitleBar` 可通过配置编辑关闭，重新打开窗口后生效；系统、DWM 或窗口条件不满足时保留原生标题栏。Spectrum 以源码链接复用主程序的 `CompactTitleBarChrome` 与可见性保护逻辑，不加载主程序可执行程序集；全屏时暂停紧凑外观，退出全屏后恢复。
