...
`ImageShaderPresentation` 拥有独立滤镜状态和 effect 附着，工具负责界面与显式持久化。滤镜的通道顺序默认 `Rgb`，可在当前视图实时切换为 `Rgb`、`Rbg`、`Grb`、`Gbr`、`Brg` 或 `Bgr`；排列名称依次表示输出 R、G、B 取自哪个当前显示通道，例如 `Brg` 把 `(R,G,B)` 显示为 `(B,R,G)`。显示白平衡提供范围为 `-1..1`、默认 `0` 的色温与色调控制，并与手动 RGB 增益组合。以上变化都不提交或改写源像素。滤镜沿用完整画布的 `SceneEffect` 范围，包含图像和叠加内容；没有静默缩小成只处理底图。关闭时仅在当前 effect 仍是自身对象时恢复先前 effect。
