...
常规页面按元数据分成带标题的卡片：主题与语言使用 `Appearance`，聚合启动更新、更新前快照和代理设置使用 `Updates`，日志级别使用 `Diagnostics`；其它设置继续沿用原分区。侧栏固定宽度，内容区限制最大宽度并居中，标题与设置内容共用纵向滚动；切换导航组或更新搜索会滚动回顶部。行标题与说明采用不同字重、颜色，分隔线在卡片内部缩进，说明仍可换行。
