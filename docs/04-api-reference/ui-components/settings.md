...
[启动页主题](./ColorVision.Themes.md#启动页主题)位于外观与语言中，使用普通下拉框选择深色、浅色或跟随软件主题；默认深色，沿用普通选项保存边界，下次启动生效。
...
常规页面按元数据分成带标题的卡片：主题、语言、启动恢复窗口、紧凑主窗口和 Windows 经典菜单使用 `Appearance`，聚合启动更新、更新前快照和代理设置使用 `Updates`，日志级别使用 `Diagnostics`；其它设置继续沿用原分区。侧栏固定宽度，内容区限制最大宽度并居中，标题与设置内容共用纵向滚动；切换导航组或更新搜索会滚动回顶部。行标题与说明采用不同字重、颜色，分隔线在卡片内部缩进，说明仍可换行。

普通行右侧的非布尔编辑器使用 200 DIP 容器，开关按内容宽度对齐。生成单个编辑器后清除原属性面板的标签间距，文本框和下拉框填满容器，不再叠加子控件最小宽度；否则 WPF 可能将超出布局槽的左边框裁切。复合属性编辑器保留其内部尺寸规则，不递归强制每个子输入框的宽度。窗口内专用输入样式不作为自定义页的隐式 ComboBox 样式，避免影响维护等页面的小型选择器。

“允许程序多开”默认关闭。该设置使用 `APPConfig.AllowMultipleInstances` 持久化；旧的 `IsMute` 键不会恢复原来的多开状态，升级后首次读取按关闭处理。
