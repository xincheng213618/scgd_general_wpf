...
公共停靠外观由 `Themes/AvalonDockTheme.cs` 和同目录 XAML 提供，主程序与 Spectrum 使用同一份深浅色模板、浮窗边框和标签命令资源。引用主题本身不创建工作区或运行初始化器；主题及兼容资源入口见[停靠外观与主题边界](../../01-user-guide/interface/main-window.md#停靠外观与主题边界)。
