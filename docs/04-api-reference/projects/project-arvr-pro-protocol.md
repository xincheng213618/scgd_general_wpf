...
项目窗口存在时不返回单独 ACK。服务端从当前执行位置之后查找下一条启用步骤，按配置切图/预处理并启动 Flow。解析成功且还有下一项时，在保存当前结果前推送下一条 `SwitchPG`，让外部 PG 准备与本地保存重叠；下一 Flow 仍需收到切图完成确认并完成当前结果收尾后才启动。RunAll 保持原有顺序。后续 `SwitchPG` 的 `MsgID` 为空，`Msg` 一般为 `Switch PG`，允许失败后继续时可能带上一流程失败说明。

最后一项完成或失败策略要求结束时推送 `ProjectARVRResult`。提前请求切图后即使结果保存失败，也不会重复发送同一步切图请求。Flow 正常完成后的切图请求若无可用连接或发送抛异常，会在当前结果处理结束后终止本轮，沿用现有 `ProjectARVRResult` 失败响应，不因“允许测试失败”而自动重发；连接已断开时仍可能无法送达最终结果。本轮结束后的切图确认会被忽略。事件名、字段、步骤索引及确认报文均保持原约定。
