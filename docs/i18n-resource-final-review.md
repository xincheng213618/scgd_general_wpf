...
### 问题描述
前一轮字符级替换脚本将中文字符替换为 `?`，导致约 2,582 个条目出现乱码式坏翻译（如 "? act ?"、"piece ?"、"Save天数"、"Exposure最小Value"）。

### 修复措施
1. 从 git 恢复所有 resx 文件到字符级替换前的干净状态
2. 重新应用针对性修复：
   - 添加缺失 key（使用正确翻译，非字符级替换）
   - 修复中文残留（使用完整翻译词典）
   - 修复混合中英文条目（如 "Save天数" → "保存日数"）
   - zh-Hant 使用 opencc 简繁转换
3. 修复约 4,200+ 条坏翻译

### 最终验证
- `??` 乱码模式：**0** 输出
- en/fr/ko/ru 中文残留：仅语言名称标签（简体中文、繁體中文、日本語），属于正常显示
- ja 文件中的 CJK 字符：均为合法日语汉字（分析、温度、目標等）
- Build：**0 errors**
