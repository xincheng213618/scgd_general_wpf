...
共 180 个主题；默认 CLI 搜索只返回 current，使用 `--all` 明确包含规划与历史。
...
- [客户项目](./domains/projects.md) — 9 个主题；客户包、业务流程、协议对接与结果留存。
