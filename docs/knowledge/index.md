...
共 175 个主题；默认 CLI 搜索只返回 current，使用 `--all` 明确包含规划与历史。
...
| 源码根 | 目录分组 | 关联主题 |
| --- | ---: | ---: |
| [ColorVision](./code/source-ColorVision.md) | 10 | 39 |
| [UI](./code/source-UI.md) | 14 | 85 |
...
- [平台与架构](./domains/platform.md) — 10 个主题；宿主架构、模块责任、扩展分流与权限边界。
...
- [构建、测试与交付](./domains/delivery.md) — 24 个主题；克隆环境、构建依赖、测试、发布脚本和更新。
