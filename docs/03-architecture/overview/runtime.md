...
related: ["platform.architecture", "platform.startup-integrity", "delivery.update", "plugins.model", "ui.discovery", "ui.wizards", "ui.localization", "ui.search", "engine.rc-registration", "flow.architecture"]
---
...
| 功能启动器 | `--feature` 先按 Header、再按类型名匹配 `IFeatureLauncher`；匹配后执行并清理启动记录，未找到则打开主窗口。Execute 返回不证明该功能后续的异步业务完成 |
