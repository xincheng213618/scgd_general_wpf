...
- 流畅的缩放、平移和适配
- 伪彩色映射和直方图分析
- 3D 可视化（CIE色彩空间、深度图、高度图3D视图）
  - **v2.0+ 优化**：3D视图支持智能网格缓存、实时高度缩放、截图导出、重置视角
...
3D 视图（`Window3D`）将当前图像作为高度图进行三维可视化，支持伪彩色映射和交互式相机控制。

**核心特性：**
- **智能下采样**：根据目标分辨率自动对原始图像进行下采样，保证大图像的流畅渲染
- **伪彩色映射**：支持 24 种内置伪彩色（jet、viridis、plasma、inferno、magma、cividis 等），右侧显示颜色条
- **实时高度缩放**：通过 `+` / `-` 按钮或键盘 `+` / `-` 实时调整高度比例
- **相机控制**：支持位置移动和视角方向调整
- **重置视角**：一键恢复初始相机位置和方向
- **截图导出**：将当前 3D 视口保存为 PNG / JPG / BMP
...
| `+` | 增加高度缩放 | `+` / `Num+` |
| `-` | 减少高度缩放 | `-` / `Num-` |
| `◀` ▲ `▶` ▼ | 相机位置移动 | `L` 左 / `T` 前 / `R` 右 / `B` 后 |
| `◀` ▲ `▶` ▼ | 视角方向调整 | `D` 左看 / `A` 上看 / `F` 右看 / `C` 下看 |
| 伪彩色下拉 | 选择颜色映射 | - |
| 重置视角 | 恢复初始相机 | `Home` |
| 截图 | 保存当前视图 | - |
| 设置 | 调整渲染分辨率 | - |
...
- **网格缓存**：3D 网格的三角索引和纹理坐标只构建一次，调整高度时仅更新顶点位置，大幅提升缩放流畅度
- **目标分辨率**：默认 512x512，可在设置中调整。分辨率越高细节越丰富，但渲染负载越大
- **冻结资源**：颜色映射纹理和 LUT 在加载时冻结，减少 UI 线程开销

### 故障排除

| 问题 | 可能原因 | 解决方案 |
|------|----------|----------|
| 3D 视图打开空白 | 图像格式不支持或下采样失败 | 确认图像为 8bit/16bit 灰度或彩色图像 |
| 缩放时卡顿 | 目标分辨率设置过高 | 在设置中将分辨率调低至 256x256 或 512x512 |
| 伪彩色不显示 | 缺少颜色映射资源文件 | 检查 `Assets/Colormap/` 目录下的图片资源 |
| 截图失败 | 文件权限或路径问题 | 选择有写入权限的目录保存 |
