...
“四色校正”可从四色资源行、窗口顶部或“应用与工具”进入。支持相机取图 / 选择 CVCIE 后直接框选 POI，以及光谱单次采集 / 选择当前设备历史结果；单点一组、MATLAB RGBW 四组、Python RGB 三组，顺序不限。Python RGB 导出独立的 XYZ 修正矩阵，单点与 RGBW 输出修正后的校正文件。IP 30%～95% 与图像校正文件来源在计算前核对；未知或异常项需明确确认，数据变化会撤销旧计算结果。上方计算后，底部可另存副本；单点与 RGBW 还可备份并替换当前文件、重启 ColorVision 服务。有限负数保留，Python RGB 不允许直接替换原文件。完整步骤、备份与重启失败处理及相机曝光验证边界见[四色校正采集](./calibration.md#四色校正采集)。
