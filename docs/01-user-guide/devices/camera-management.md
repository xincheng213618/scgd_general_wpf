...
“四色校正”可从四色资源行、窗口顶部或“应用与工具”进入。支持相机取图 / 选择 CVCIE 后直接框选 POI，以及光谱单次采集 / 选择当前设备历史结果；单点一组，RGBW 四组，顺序不限。IP 30%～95% 与图像校正文件来源在计算前核对；未知或异常项需明确确认，数据变化会撤销旧计算结果。结果只另存副本，有限负数保留。完整步骤、异常规则及相机曝光验证边界见[四色校正采集](./calibration.md#四色校正采集)。
