...
选择结果后的显示链是 `ViewResultImage.FileUrl → OpenImage(string?) → ImageView.OpenImage(filePath)`，空路径会清空图像。本地流程通知也先按 `MasterId` 查主记录，尚未把流程内存帧送入设备结果视图。因此 `SaveFiles=false` 的流程帧可供下游算法使用，但不能据此在 `ViewCamera` 预览或重新打开无文件历史结果；直接显示内存图像的是前述本地相机管理窗口。若 16 位三通道文件与本地相机预览的颜色观感不同，可在“图像设置 → 当前视图 → 显示滤镜”实时纠正：整体偏绿或偏洋红优先使用“色调”，冷暖偏移使用“色温”；只有确认数据通道位置错乱时才选择对应的六种“通道顺序”。相机回调通过通用 `RealtimeFramePresenter.GetPixelFormat` 选择像素格式并直接提交 SDK 原始帧，不执行 `CvtColor` 或逐像素交换；文件打开也不提供专用的“本地相机顺序”开关。如后续需要在数据入口固定纠正，应基于真实设备样本另行增加配置。设备级无文件预览见[待实施设计](../../02-developer-guide/engine-development/local-camera-memory-preview.md)。
