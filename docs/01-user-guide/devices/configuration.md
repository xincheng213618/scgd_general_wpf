...
设备树右侧的属性操作由[通用命令生成器](../../04-api-reference/ui-components/property-grid.md#命令属性页自动生成)依据命令元数据生成，按设备与连接、校准与校正、采集与显示、维护与诊断归类。不同设备使用同一套按钮布局和主题规则，仅显示自身拥有的命令；修改配置、文件保存、重启、重置和删除来自基类元数据。页面不会因展示按钮而自动执行设备操作。
