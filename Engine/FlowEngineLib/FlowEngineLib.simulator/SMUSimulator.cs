...
internal class SMUSimulator : BaseSimulator
{
	...
	protected override CVMQTTRequest GetResponseEvent(MQActionEvent msg)
	{
		...
		if ("StartMeasureData".Equals(msg.EventName))
		{
			...
			result = new CVMQTTRequest(nodeCode, deviceCode, "Finish", cVMQTTRequest.SerialNumber, data, string.Empty, -1);
		}
		else if ("SetMeasureData".Equals(msg.EventName))
		{
			...
			result = new CVMQTTRequest(nodeCode, deviceCode, "Finish", cVMQTTRequest2.SerialNumber, data2, string.Empty, -1);
		}
		return result;
	}
}
