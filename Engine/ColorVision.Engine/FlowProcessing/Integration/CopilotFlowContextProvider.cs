...
namespace ColorVision.Engine.FlowProcessing.Integration
{
    public sealed class CopilotFlowContextProvider : ICopilotContextProvider
    {
        ...
        public static CopilotFlowContextProvider Create(FlowCopilotService service)
        {
            ArgumentNullException.ThrowIfNull(service);
            return new CopilotFlowContextProvider(
                cancellationToken => CaptureSnapshotAsync(service, cancellationToken),
                () => ReferenceEquals(FlowEngineManager.Current?.Copilot, service),
        ...
        private static async Task<CopilotFlowContextSnapshot?> CaptureSnapshotAsync(
            FlowCopilotService service,
            CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            var dispatcher = service.Manager.View?.Dispatcher ?? Application.Current?.Dispatcher;
