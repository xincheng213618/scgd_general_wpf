...
namespace ColorVision.Engine.FlowProcessing.PreProcess
{
    ...
    public class ScriptPreProcessorConfig : PreProcessConfigBase
    {
    ...
    public class ScriptPreProcessor : PreProcessorBase<ScriptPreProcessorConfig>
    {
        private const string DefaultCmdExecutable = "cmd.exe";
        private const string DefaultPowerShellExecutable = "powershell.exe";
        private static readonly ILog log = LogManager.GetLogger(typeof(ScriptPreProcessor));
        ...
        public override async Task<bool> PreProcess(PreProcessContext ctx)
        {
            ...
            log.Info($"ScriptPreProcessor: 执行 {displayCommand}");
            ...
            try
            {
                ...
                if (!process.Start())
                {
                    return HandleFailure($"ScriptPreProcessor: {Properties.Resources.CommandLineScript_StartFailed}");
                }
                ...
                if (timedOut)
                {
                    return HandleFailure($"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_Timeout, Config.TimeoutMs)}");
                }

                if (IsSuccessExitCode(process.ExitCode))
                {
                    log.Info($"ScriptPreProcessor: 执行完成，ExitCode={process.ExitCode}，耗时 {stopwatch.ElapsedMilliseconds} ms");
                    return true;
                }
                ...
                string message = string.IsNullOrWhiteSpace(detail)
                    ? $"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_ExecutionFailed, process.ExitCode)}"
                    : $"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_ExecutionFailedWithDetail, process.ExitCode, TrimMessage(detail, 240))}";
                return HandleFailure(message);
            }
            catch (Exception ex)
            {
                stopwatch.Stop();
                return HandleFailure($"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_ExecutionException, ex.Message)}");
            }
        }

        private bool TryBuildProcessStartInfo(PreProcessContext ctx, out ProcessStartInfo startInfo, out string displayCommand, out string errorMessage)
        {
            startInfo = null!;
            ...
            {
                errorMessage = $"ScriptPreProcessor: {Properties.Resources.CommandLineScript_FileAndCommandEmpty}";
                return false;
            }
            ...
            {
                errorMessage = $"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_WorkingDirectoryNotFound, workingDirectory)}";
                return false;
            }
        ...
        private static void LogOutput(string stdout, string stderr)
        {
            ...
            {
                log.Info($"ScriptPreProcessor stdout: {TrimMessage(stdout.Trim(), 2000)}");
            }

            if (!string.IsNullOrWhiteSpace(stderr))
            {
                log.Warn($"ScriptPreProcessor stderr: {TrimMessage(stderr.Trim(), 2000)}");
            }
        }

        private static string ExpandPlaceholders(string value, PreProcessContext ctx)
        {
        ...
        private static bool ValidateScriptFile(PreProcessScriptType scriptType, string scriptFile, string workingDirectory, out string errorMessage)
        {
            ...
            errorMessage = $"ScriptPreProcessor: {string.Format(Properties.Resources.CommandLineScript_FileNotFound, scriptFile)}";
            return false;
        }
