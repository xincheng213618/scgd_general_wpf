...
namespace ColorVision.Engine.FlowProcessing.PostProcess
{
    public class TypeNameConverter : IValueConverter
    {
        ...
        {
            if (value is IPostProcessor process)
            {
    ...
    /// <summary>
    /// PostProcessManagerWindow.xaml 的交互逻辑
    /// </summary>
    public partial class PostProcessManagerWindow : Window
    {
        private PostProcessMeta _currentSelectedMeta;
        ...
        private void RefreshPropertyPanel()
        {
            ...
            var manager = DataContext as PostProcessManager;
            ...
            var config = selectedMeta.PostProcessor?.GetConfig();
            if (config != null)
            {
        ...
        {
            // Refresh when PostProcessor changes (which may change the config)
            if (e.PropertyName == nameof(PostProcessMeta.PostProcessor))
            {
                RefreshPropertyPanel();
            }
        }

        private void AddMetaInfoSection(PostProcessMeta meta)
        {
        ...
        private void AddConfigSection(object config, PostProcessMeta meta)
        {
            var border = new Border
            {
                BorderBrush = (System.Windows.Media.Brush)FindResource("BorderBrush"),
        ...
        private static void AddLabeledTextBox(StackPanel parent, string label, string value, Action<string> onChanged)
        {
            var dock = new DockPanel { Margin = new Thickness(0, 0, 0, 6) };
            dock.Children.Add(new TextBlock 
            { 
