...
namespace ColorVision.Engine.FlowProcessing.PostProcess.IVL
{
    ...
    public partial class ILvPlotWindow : Window
    {
        ...
        private void LoadData(List<SMUResultModel> smuResults, List<PoiResultCIExyuvData> poixyuvDatas, ObservableCollection<ViewResultSpectrum> spectrumResults)
        {
            ...
            if (hasPoiData)
            {
                // Calculate how many POIs per SMU measurement (same logic as IvlPostProcessor)
                int poisPerMeasurement = poiCount / smuCount;
                if (poisPerMeasurement == 0)
                    poisPerMeasurement = 1;

                // Match the data pairing logic from IvlPostProcessor.cs
                for (int i = 0; i < poiCount; i++)
                {
                    // Calculate SMU index: z = i / cout (from IvlPostProcessor.cs)
            ...
            if (hasSpectrumData)
            {
                ...
                // Match spectrum results with SMU data (similar to IvlPostProcessor logic)
