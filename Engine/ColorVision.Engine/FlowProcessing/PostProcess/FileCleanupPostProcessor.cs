...
namespace ColorVision.Engine.FlowProcessing.PostProcess
{
    ...
    /// </summary>
    public class FileCleanupPostProcessorConfig : ViewModelBase
    {
    ...
    [PostProcess("文件清理", "删除流程中产生的临时文件或指定文件")]
    public class FileCleanupPostProcessor : PostProcessorBase<FileCleanupPostProcessorConfig>
    {
        private static readonly ILog log = LogManager.GetLogger(typeof(FileCleanupPostProcessor));
        ...
        public override bool Process(PostProcessContext ctx)
        {
            if (!Config.Enabled)
            {
                log.Info("FileCleanupPostProcessor 未启用，跳过");
                return true;
            }

            if (string.IsNullOrWhiteSpace(Config.FolderPath))
            {
                log.Warn("FileCleanupPostProcessor: 文件夹路径未配置");
                return true; // Don't fail the batch
            }

            if (!Directory.Exists(Config.FolderPath))
            {
                log.Warn($"FileCleanupPostProcessor: 文件夹不存在 {Config.FolderPath}");
                return true; // Don't fail the batch
            }
            ...
            try
            {
                ...
                if (files.Count == 0)
                {
                    log.Info($"FileCleanupPostProcessor: 文件夹中没有匹配的文件 {Config.FolderPath}");
                    return true;
                }
                ...
                if (Config.DeleteOlderThanDays > 0)
                {
                    ...
                    if (files.Count == 0)
                    {
                        log.Info($"FileCleanupPostProcessor: 没有早于 {Config.DeleteOlderThanDays} 天的文件");
                        return true;
                    }
                }
                ...
                {
                    log.Info("FileCleanupPostProcessor: 根据保留规则，没有文件需要删除");
                    return true;
                }
                ...
                log.Info($"FileCleanupPostProcessor: 清理完成。删除了 {deletedCount} 个文件，释放了 {deletedSize / (1024 * 1024)}MB");
            ...
            {
                log.Error("FileCleanupPostProcessor 执行失败", ex);
        ...
        private static void DeleteEmptyFolders(HashSet<string> foldersToCheck, string rootFolder)
        {
            ...
            {
                log.Info($"FileCleanupPostProcessor: 删除了 {deletedFolderCount} 个空文件夹");
            }
        }
