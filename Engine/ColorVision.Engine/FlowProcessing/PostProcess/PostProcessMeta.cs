...
namespace ColorVision.Engine.FlowProcessing.PostProcess
{
    ...
    public class PostProcessMeta:ViewModelBase
    {
        ...
        private PostProcessMetadata? _cachedMetadata;

        public string ProcessTypeName => PostProcessor?.GetType().Name ?? string.Empty;
        public string ProcessTypeFullName => PostProcessor?.GetType().FullName ?? string.Empty;

        /// <summary>
        /// Gets a value indicating whether this batch process has a configurable config.
        /// </summary>
        [JsonIgnore]
        public bool HasConfig => PostProcessor?.GetConfig() != null;
        ...
        private void EditConfig()
        {
            ...
            var config = PostProcessor.GetConfig();
        ...
        {
            if (PostProcessor != null && !string.IsNullOrEmpty(ConfigJson))
            {
                PostProcessor.SetConfig(ConfigJson);
            }
        }
        ...
        public string ExecutionOrder
        {
            get
            {
                var manager = PostProcessManager.GetInstance();
        ...
        /// </summary>
        [JsonIgnore]
        public PostProcessMetadata Metadata
        {
            get
            {
                if (_cachedMetadata == null && PostProcessor != null)
                {
                    _cachedMetadata = PostProcessMetadata.FromProcess(PostProcessor);
                }
                return _cachedMetadata ?? new PostProcessMetadata();
            }
        }
