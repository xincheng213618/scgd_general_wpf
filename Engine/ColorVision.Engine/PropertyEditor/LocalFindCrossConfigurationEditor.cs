...
public sealed class LocalFindCrossConfigurationEditor : IPropertyEditor
{
    public DockPanel GenProperties(PropertyInfo property, object obj)
    {
        ...
        button.Click += (_, _) =>
        {
            var window = new LocalFindCrossConfigurationWindow(property.GetValue(obj) as string ?? "")
            {
    ...
    private sealed class SummaryConverter : IValueConverter
    {
        ...
        {
            if (!LocalFindCrossConfigurationDraft.TryCreate(value as string ?? "", out var draft, out _) || !draft!.TryGetJson(out _, out _)) return "配置有误 · 点击编辑";
            return $"角度 {draft.ExpectedAngle}° · 偏差 ±{draft.AngleTolerance}°";
        }
