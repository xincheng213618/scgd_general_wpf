...
namespace cvColorVision
{
    ...
    {
        Keithley_2400 = 0,
        kethiley2400 = Keithley_2400,
        Keithley_2600 = 1,
        kethiley2600 = Keithley_2600,
        Precise_S100 = 2,
        PssSx00Dll = Precise_S100,
        Vxi11Protocol = 3,
        VictualPss = 4
    }
    ...
    public class PassSx
    {
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvPss_Sx_GetIDN(int nDevID, [Out] byte[] pszIdn, ref int strLen);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvPssSxSetSourceVRaw(int nDevID, [MarshalAs(UnmanagedType.I1)] bool isSourceV);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvPss_Sx_Set4WireFront(int nDevID, [MarshalAs(UnmanagedType.I1)] bool bWier, [MarshalAs(UnmanagedType.I1)] bool bFront);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvPss_Sx_SetSrcAorB(int nDevID, [MarshalAs(UnmanagedType.I1)] bool bSrcA);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvStepMeasureDataExRaw(int nDevID, double srcRng, double lmtRng, double measureVal, double lmtVal, ref double rstV, ref double rstI);
        public static bool cvStepMeasureDataEx(int nDevID, double srcRng, double lmtRng, double measureVal, double lmtVal, ref double rstV, ref double rstI)
        {
            ...
            else return cvStepMeasureDataExRaw(nDevID, srcRng / 1000.0, lmtRng, measureVal / 1000.0, lmtVal, ref rstV, ref rstI);
        }

        // 序列扫描 指定量程
        [DllImport(LIBRARY_CVCAMERA, EntryPoint = "cvSweepData", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.StdCall)]
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvSweepDataRaw(int nDevID, double srcRng, double lmtRng, double lmtVal, double startVal, double stopVal, int points, [Out] double[] pVList, [Out] double[] pIList);
        ...
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvSweepDataAutoRngRaw(int nDevID, double lmtVal, double startVal, double stopVal, int points, [Out] double[] pVList, [Out] double[] pIList);
        public static bool cvSweepDataAutoRng(int nDevID, double lmtVal, double startVal, double stopVal, int points, double[] pVList, double[] pIList)
        {
            ...
            else return cvSweepDataAutoRngRaw(nDevID, lmtVal, startVal / 1000.0, stopVal / 1000.0, points, pVList, pIList);
        }

        // 序列扫描 自定义值列表
        [DllImport(LIBRARY_CVCAMERA, EntryPoint = "cvListData", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.StdCall)]
        [return: MarshalAs(UnmanagedType.Bool)]
        private static extern bool cvListDataRaw(int nDevID, double srcRng, double lmtRng, double lmtVal, [In] double[] pCustomData, int points, [Out] double[] pVList, [Out] double[] pIList);
