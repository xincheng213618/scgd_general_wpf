...
2. 归属不清时从[知识地图](docs/knowledge/index.md)或本地搜索定位主题；已知归属时直接读取相关契约、实现和测试。行为或契约变化但文档归属不清、源码或测试路径移动/删除时，用 `knowledge.mjs impact <仓库相对路径>` 查候选。
3. 实现一个明确目的，沿用现有 MVVM、服务、模板、插件及属性编辑器边界。强名称、运行时依赖和平台要求按项目文件与 AGENTS 维护，不用临时关闭检查来隐藏问题。
4. 行为、契约、操作或架构边界变化时同步所属主题；内部重构或恢复既有契约的修复，在已有知识、实现入口与验证说明仍准确时无需改正文，否则只修正受影响位置。路径移动仍须修复引用。按下表选择最小验证集，区分构建、测试、启动、设备操作和发布。
...
| 知识内容、文档工具和页面 | [知识维护规范](docs/knowledge/maintenance.md#按变更选择验证)：按正文、元数据、链接/导航或工具变更选择检查；目录输入变化时才生成 |
...
- 按知识变更范围同步正文和必要的生成目录；没有知识变化时无需为提交补写文档。不手工改 catalog、源码地图或站点导航。
