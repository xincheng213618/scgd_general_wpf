...
- `Scripts\release.bat` is the only normal main-release entry point. For main-app/repository-wide “发布”, “打包发布”, “直接打包”, or “快速发布”, use an explicitly requested version when provided; otherwise read the current remote version and increment its final numeric component by one. Update `Directory.Build.props` `VersionPrefix` and `CHANGELOG.md` first, run the wrapper once from the canonical worktree, and report its result. Zero exit completes artifact publication; pushing commits or tags remains separate unless explicitly requested.
- When release preparation finds many intended uncommitted changes, organize the changelog and create source commits before running the wrapper. For a plugin or project release, commit only that `Plugins/<Name>/` or `Projects/<Name>/` scope as its own change and preserve unrelated work. For a main-app/repository-wide release, commit all intended source, test, script, and documentation changes together as one main-program change, including intended plugin/project changes delivered with the main program. After the wrapper succeeds, create one separate `release: <version>` commit for the release metadata, then create an annotated local `v<version>` tag on that commit. Use exact path or hunk allowlists throughout; do not push the branch or tag unless explicitly requested.
- The wrapper owns build, installer validation, upload and compact parallel acceptance (signature, remote version/changelog, installer/update download sizes, Git status). The external Advanced Installer project syncs version and files from the canonical worktree. Do not add standalone tests, pre-builds, deep history review, duplicate remote/download checks, an isolated release worktree, or another packaging entry point; `Scripts\build.py` and `Scripts\build_update.py` are internal steps, never local-only shortcuts.
