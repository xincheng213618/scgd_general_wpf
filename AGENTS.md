...
- Keep quick-release preparation minimal: update `VersionPrefix`, add a new top-level `CHANGELOG.md` section containing one to three short user-facing items for only the current release, run `python Scripts\regroup_changelog.py --write`, then run `Scripts\release.bat` once and report its result. The regrouping script archives every exact release in `docs/_history/CHANGELOG.md` before rewriting the public root file to contain only the newest release. Archived versions are immutable and the underscored history directory is for internal review only; it is excluded from the public documentation build and is not uploaded as the main application changelog. Omit tests, documentation, internal implementation details, file/class names, refactoring mechanics, and commit-by-commit narration from new public release notes.
