...
- `Scripts\release.bat` is the only normal main-release entry point. For main-app/repository-wide “发布”, “打包发布”, “直接打包”, or “快速发布”, use an explicitly requested version when provided; otherwise read the current remote version and increment its final numeric component by one. Update `Directory.Build.props` `VersionPrefix` and `CHANGELOG.md` first, run the wrapper once from the canonical worktree, and report its result. Zero exit completes quick release; commit/push is separate unless explicitly requested.
