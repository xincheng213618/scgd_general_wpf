...
public sealed class CopilotKeyboardShortcutHelpTests
{
    ...
    public void ReportExplainsFocusSpecificBindingsAndSafety()
    {
        ...
        Assert.Contains("Enter — 标准模式发送；多行模式插入换行", report);
        Assert.Contains("Shift+Enter — 标准模式插入换行；多行模式发送", report);
        Assert.Contains("Ctrl+Enter — 空闲时发送；Agent 运行中取消当前轮并把输入提升为下一轮", report);
        Assert.Contains("PgUp / PgDn — 输入框无法继续滚动时按页翻阅对话", report);
        Assert.Contains("Esc — 停止当前回复；Agent 有安全检查点时先暂停，再按一次取消", report);
        Assert.Contains("Esc Esc — 打开会话回溯点清单，不立即回溯", report);
