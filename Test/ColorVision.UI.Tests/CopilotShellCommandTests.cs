...
public sealed class CopilotShellCommandTests : IDisposable
{
    ...
    [Fact]
    public void RegistryKeepsShellAvailableOutsideChatMode()
    {
        var registry = CopilotToolRegistry.CreateDefault();

        Assert.Contains(registry.FindTools(Request("我想要知道6666端口有没有被占用")), tool => tool.Name == "RunShellCommand");
        Assert.Contains(registry.FindTools(Request("解释一下畸变校正")), tool => tool.Name == "RunShellCommand");
        Assert.Contains(registry.FindTools(Request("我想要知道6666端口有没有被占用", hasHistory: true)), tool => tool.Name == "RunShellCommand");
