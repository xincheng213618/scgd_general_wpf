...
def main():
    p=argparse.ArgumentParser(); p.add_argument('image'); p.add_argument('output'); p.add_argument('--rois',required=True); p.add_argument('--encoding',default='unknown',choices=['unknown','linear','srgb']); p.add_argument('--dll',type=Path,help='Explicit native build to verify; its hash is recorded.')
    p.add_argument('--chart-type',default='bmw',choices=['bmw','checkerboard','auto']); args=p.parse_args()
    ...
    if args.chart_type!='bmw': dll.M_LocateSfrTargetV1.argtypes=[Image,Rect,C.c_int,C.POINTER(C.c_void_p)]
    ...
    overlay=pixels.copy() if pixels.dtype==np.uint8 else cv2.normalize(pixels,None,0,255,cv2.NORM_MINMAX,cv2.CV_8U)
    if channels==1: overlay=cv2.cvtColor(overlay,cv2.COLOR_GRAY2BGR)
    results=[]
    ...
    for index,coords in enumerate(json.loads(args.rois)):
        roi=Rect(*coords)
        result=call(dll.M_LocateBmwTargetV1,image,roi) if args.chart_type=='bmw' else call(dll.M_LocateSfrTargetV1,image,roi,1 if args.chart_type=='checkerboard' else 2)
    ...
    (output/'results.json').write_text(json.dumps(dict(source=str(source),sourceSha256=before,dllSha256=hashlib.sha256(dllpath.read_bytes()).hexdigest(),sourceWidth=image.cols,sourceHeight=image.rows,chartType=args.chart_type,options=options,results=results),indent=2),encoding='utf-8')
