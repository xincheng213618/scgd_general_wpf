...
namespace ColorVision.Rbac.Services
{
    public class PermissionService : IPermissionService
    {
        ...
        public void EnsureSeed()
        {
            var seeds = GetSeedPermissions();
            var codes = seeds.Select(s=>s.Code).ToList();
            var exists = _db.Queryable<PermissionEntity>().Where(p=>codes.Contains(p.Code)).Select(p=>p.Code).ToList();
            var toInsert = seeds.Where(s=>!exists.Contains(s.Code)).ToList();
            if (toInsert.Count>0)
                _db.Insertable(toInsert).ExecuteCommand();
        }

        private static List<PermissionEntity> GetSeedPermissions()
        {
