...
本目录通过矩形右键或“算法调用”（`AlgorithmsCall`）菜单，直接调用 `OpenCVMediaHelper.M_FindLightBeads` 并向当前图像画布追加圆标注；不是 Engine 的远端 LED 模板，也不经过统一算法 Runner 的 provider 门禁。
...
- `Execute` 返回不表示标注完成。UI 追加红色亮点圆、黄色暗区候选圆，不显示统计框，也不自动保存源图或业务结果。
