...
{
    public partial class LineFitResultWindow : Window
    {
        private readonly AlgorithmResult _result;
        private readonly IDisposable _overlaySession;
        private bool _disposed;

        public LineFitResultWindow(AlgorithmResult result, ImageProcessingContext image, DrawEditorContext draw)
        {
            ArgumentNullException.ThrowIfNull(result);
            AlgorithmTableArtifact points = result.GetArtifact<AlgorithmTableArtifact>("line-fit-points")
                ?? throw new ArgumentException("The result has no line-fit point table.", nameof(result));
            InitializeComponent();
            _result = result;
            _overlaySession = AlgorithmOverlayRenderer.Apply(image, draw, result);
            PointsGrid.ItemsSource = ToTable(points).DefaultView;
            bool accepted = Measurement("line_fit.accepted") == 1;
            SummaryText.Text = accepted
                ? $"有效点：{Measurement("line_fit.inlier_count"):N0}/{Measurement("line_fit.point_count"):N0}；角度：{Measurement("line_fit.angle_degrees"):G8}°；RMS 残差：{Measurement("line_fit.rms_residual"):G8}px；质量：{Measurement("line_fit.confidence"):G6}。置信度是确定性的拟合质量分数，不是统计概率。"
                : "拟合被结构化拒绝；请查看各点 RejectionReason。";
            Closed += (_, _) => DisposeOwnedState();
        }

        private double Measurement(string name)
            => _result.GetArtifact<AlgorithmMeasurementArtifact>("line-fit-summary")!.Measurements.Single(item => item.Name == name).Value;
        ...
        {
            SaveFileDialog dialog = new() { Filter = "CSV 文件 (*.csv)|*.csv", FileName = "line-fit.csv", AddExtension = true };
        ...
        {
            SaveFileDialog dialog = new() { Filter = "JSON 文件 (*.json)|*.json", FileName = "line-fit.json", AddExtension = true };
            if (dialog.ShowDialog(this) != true) return;
            try
            {
