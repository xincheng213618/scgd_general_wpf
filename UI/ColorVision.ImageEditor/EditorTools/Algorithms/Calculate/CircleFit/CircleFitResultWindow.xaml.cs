...
{
    public partial class CircleFitResultWindow : Window
    {
        private readonly AlgorithmResult _result;
        private readonly IDisposable _overlaySession;
        private bool _disposed;

        public CircleFitResultWindow(AlgorithmResult result, ImageProcessingContext image, DrawEditorContext draw)
        {
            ArgumentNullException.ThrowIfNull(result);
            AlgorithmTableArtifact points = result.GetArtifact<AlgorithmTableArtifact>("circle-fit-points")
                ?? throw new ArgumentException("The result has no circle-fit point table.", nameof(result));
            InitializeComponent();
            _result = result;
        ...
        private double Measurement(string name)
            => _result.GetArtifact<AlgorithmMeasurementArtifact>("circle-fit-summary")!.Measurements.Single(item => item.Name == name).Value;
        ...
        {
            SaveFileDialog dialog = new() { Filter = "CSV 文件 (*.csv)|*.csv", FileName = "circle-fit.csv", AddExtension = true };
        ...
        {
            SaveFileDialog dialog = new() { Filter = "JSON 文件 (*.json)|*.json", FileName = "circle-fit.json", AddExtension = true };
            if (dialog.ShowDialog(this) != true) return;
            try
            {
