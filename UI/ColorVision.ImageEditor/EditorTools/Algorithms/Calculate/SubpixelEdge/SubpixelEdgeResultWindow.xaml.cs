...
{
    public partial class SubpixelEdgeResultWindow : Window
    {
        private readonly AlgorithmResult _result;
        private readonly IDisposable _overlaySession;
        private bool _disposed;

        public SubpixelEdgeResultWindow(AlgorithmResult result, ImageProcessingContext image, DrawEditorContext draw)
        {
            ArgumentNullException.ThrowIfNull(result);
            AlgorithmTableArtifact edges = result.GetArtifact<AlgorithmTableArtifact>("subpixel-edges")
                ?? throw new ArgumentException("The result has no subpixel-edge table.", nameof(result));
            InitializeComponent();
            _result = result;
        ...
        private double Measurement(string name)
            => _result.GetArtifact<AlgorithmMeasurementArtifact>("subpixel-edge-summary")!.Measurements.Single(item => item.Name == name).Value;
        ...
        {
            SaveFileDialog dialog = new() { Filter = "CSV 文件 (*.csv)|*.csv", FileName = "subpixel-edges.csv", AddExtension = true };
        ...
        {
            SaveFileDialog dialog = new() { Filter = "JSON 文件 (*.json)|*.json", FileName = "subpixel-edges.json", AddExtension = true };
            if (dialog.ShowDialog(this) != true) return;
            try
            {
