...
{
    public class BezierCurveProperties : BaseProperties, IBezierCurve, ICompactInspectorProvider
    {
        [Browsable(false)]
        public Pen Pen
        {
            ...
            set
            {
                _Pen = value ?? new Pen(DefaultBrush, 1);
                OnPropertyChanged();
                OnPropertyChanged(nameof(Brush));
                OnPropertyChanged(nameof(StrokeThickness));
            }
        }
        private Pen _Pen = new Pen(DefaultBrush, 1);

        [DisplayName("颜色")]
        public Brush Brush
        {
            get => Pen?.Brush ?? DefaultBrush;
            set
            {
                Brush next = value ?? DefaultBrush;
