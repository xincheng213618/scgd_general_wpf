...
namespace ColorVision.Common.NativeMethods
{
    public class Win32DeviceMgmt
    {
            ...
            for (int index = 0; index < guids.Length; index++)
            {
                ...
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        // 只有当 Name 包含 COM 时（说明确实是个串口），才加到列表里
                        if (!string.IsNullOrEmpty(deviceInfo.name) && deviceInfo.name.Contains("COM"))
                        {
        ...
        private static string GetDeviceName(IntPtr pDevInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            ...
            if (hDeviceRegistryKey == IntPtr.Zero) return "";
            ...
            try
            {
                ...
                if (result != 0 || length <= utf16terminatorSize_bytes) return "";
            }
            finally
            {
        ...
        private static string GetDeviceDescription(IntPtr hDeviceInfoSet, SP_DEVINFO_DATA deviceInfoData)
        {
            ...
            if (!success || RequiredSize <= utf16terminatorSize_bytes) return "";
