...
当前行为与限制统一维护在[共享接口、属性通知与粗粒度权限](../../docs/04-api-reference/ui-components/ColorVision.Common.md)。尤其注意：属性通知和命令不会自动切换线程，`Execute` 不替调用方检查 `CanExecute`；全局权限模式不等于 RBAC 登录或权限码检查。
