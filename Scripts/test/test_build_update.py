...
class BuildUpdateTests(unittest.TestCase):
    ...
    def test_incremental_zip_contains_only_new_or_changed_files(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with zipfile.ZipFile(old_zip, "w", zipfile.ZIP_DEFLATED) as archive:
                archive.writestr("Same.dll", b"same")
                archive.writestr("Changed.dll", b"old")
            for file_name, content in {
                "Same.dll": b"same",
                "Changed.dll": b"new",
                "New.dll": b"new",
            }.items():
                with open(os.path.join(new_version_dir, file_name), "wb") as file:
                    file.write(content)
            ...
            with zipfile.ZipFile(incremental_zip, "r") as archive:
                self.assertEqual(set(archive.namelist()), {"Changed.dll", "New.dll"})
