...
TOOL_DIRECTORY = "Tool/ProjectARVRPro.SemiAuto"
PROTOCOL_VERSION = "1.0"
DEFAULT_RELEASE_NOTES = (
    "提供 ARVR TCP 调用与 GECS PG 半自动联动，默认配置 3 条 SwitchPG 切图映射，支持现场增删、结果解析和 CSV 导出。"
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build, validate, and publish ProjectARVRPro SemiAuto."
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
...
def copy_product_sources(project_dir: Path, package_dir: Path) -> None:
    for name in (
        "ProjectARVRPro.SemiAuto.csproj",
        "Program.cs",
        "MainWindow.xaml",
        "MainWindow.xaml.cs",
        "PackageIcon.png",
    ...
    for directory_name in ("Contracts", "Automation", "GECS", "Profiles"):
        source_directory = project_dir / directory_name
        target_directory = package_dir / directory_name
...
def validate_zip(zip_path: Path, expected_root: str) -> None:
    ...
    required = {
        f"{expected_root}/ProjectARVRPro.SemiAuto.exe",
        f"{expected_root}/ProjectARVRPro.SemiAuto.csproj",
        ...
        f"{expected_root}/Automation/IntegrationProfile.cs",
        f"{expected_root}/GECS/GecsProtocol.cs",
        f"{expected_root}/Profiles/semi-auto-profile.sample.json",
    }
...
def build_package(
    ...
    project_dir = repo_root / "Projects" / "ProjectARVRPro.SemiAuto"
    project_file = project_dir / "ProjectARVRPro.SemiAuto.csproj"
    plugin_file = repo_root / "Projects" / "ProjectARVRPro" / "ProjectARVRPro.csproj"
    product_version = read_version(project_file)
    plugin_version = read_version(plugin_file)
    publish_dir = work_dir / "publish"
    package_dir = work_dir / "package" / "ProjectARVRPro.SemiAuto"
    ...
    copy_product_sources(project_dir, package_dir)

    sample_path = package_dir / "Samples" / "project-arvr-result.json"
    product_exe = package_dir / "ProjectARVRPro.SemiAuto.exe"
    run_checked(
        [
            str(product_exe),
            "--parse-file",
    ...
    if not list(smoke_dir.glob("*_items.csv")):
        raise RuntimeError("SemiAuto smoke test did not produce a CSV result.")
    run_checked(
        [
            str(product_exe),
            "--validate-profile",
            str(package_dir / "Profiles" / "semi-auto-profile.sample.json"),
        ],
        cwd=package_dir,
    )
    ...
    zip_name = f"ProjectARVRPro.SemiAuto-{product_version}.zip"
...
def publish(zip_path: Path, metadata_path: Path, metadata: dict, args: argparse.Namespace) -> None:
    ...
    if not upload_file(zip_path, settings, session=session):
        raise RuntimeError("SemiAuto ZIP upload failed.")
...
def main() -> int:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="project-arvrpro-semiauto-") as temp_dir:
