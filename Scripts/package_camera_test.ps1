...
4. 使用相机时，沿用现有相机驱动与相应 SDK 授权，在面板选择型号，在“采集参数”中设置模式，刷新后选择相机 ID，再连接。SDK 配置随程序提供。
5. 实时分析用于调焦；停止后可记录设备编号/SN、操作员、批次、参数、原图和结果。
6. 默认存档目录为 %LOCALAPPDATA%\ColorVision\CameraTest\Archive，也可在“存档信息”中选择目录。
7. “视图 → BMW 测量与显示”（或工具栏“测量设置”）可选 MTF50、MTF10、指定频率 MTF（默认 0.25）和 MTF@0.5，并控制刃边虚线。
8. 同一设置窗口的“四边测量框”分组可设置沿刃边长度、跨刃边宽度及距靶标中心距离，单位为原图像素；0 保留自动值，确认后重新分析。
9. “色差”显示 R−G、R−B、G−B 刃边位移；缺测会列出原因，不代表位移为零。
10. “清晰度”页的 Y/R/G/B 勾选同步筛选曲线和结果列表，默认每条边一行，勾选“明细”可查看完整通道指标；导出与存档保留完整计算数据。
11. 图像底部 POI 模板框旁可打开模板管理或保存当前测量框；无可用 MySQL 时自动使用本地 SQLite，无需单独安装数据库。
12. 点击通道数值或上方四边读数切换曲线；支持按点位筛选、定位最低值。图上标注与右侧使用同一指标，“专注当前点”减少文字遮挡。
13. 定位失败时点击“处理未定位”，查看原因并扩大选区重试；扩大选区支持撤销。
