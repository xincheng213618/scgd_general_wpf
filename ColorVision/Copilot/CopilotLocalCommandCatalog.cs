...
namespace ColorVision.Copilot
{
    ...
    public static class CopilotLocalCommandCatalog
    {
        ...
        [
            ...
            new("/agents", "查看请求级子代理、单次结果与预算，或按 run_id 引导、停止子代理；父 Agent 继续运行", CopilotLocalCommandKind.Subagents, AcceptsArguments: true, AvailableWhileAgentRuns: true, Usage: "/agents [roles|runs [N]|show <run_id>|steer <run_id> <message>|stop <run_id>]", Arguments:
            [
            ...
            new("/subagents", "同 /agents；查看请求级子代理与单次结果，或按 run_id 引导、停止并让父 Agent 继续", CopilotLocalCommandKind.Subagents, AcceptsArguments: true, AvailableWhileAgentRuns: true, Usage: "/subagents [roles|runs [N]|show <run_id>|steer <run_id> <message>|stop <run_id>]", Arguments:
            [
                new("roles", "只显示内置子代理角色及其只读能力"),
                new("runs", "显示当前会话最近 N 次子代理运行", AcceptsArguments: true),
                new("show", "按 run_id 显示单次子运行的限长结果与审计详情", AcceptsArguments: true),
            ...
            new("/suggestions", "管理本地历史补全和可选的轮次结束后下一请求预测", CopilotLocalCommandKind.PromptSuggestions, AcceptsArguments: true, AvailableWhileAgentRuns: true, Usage: "/suggestions [on|off|predict-on|predict-off|predict-model [current|Profile]]", Arguments:
            [
                new("on", "输入普通请求时显示当前会话的历史前缀补全"),
                new("off", "隐藏本地历史前缀补全"),
                new("predict-on", "启用轮次结束预测；仅在已选择可用预测 Profile 时调用"),
                new("predict-off", "停止并隐藏下一请求预测，不影响本地历史补全"),
                new("predict-model", "查看或选择专用于下一请求预测的模型 Profile"),
            ]),
