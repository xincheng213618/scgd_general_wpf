...
namespace ColorVision.Copilot
{
    internal static class CopilotWorkspacePatchSchemas
    {
        ...
        public static CopilotToolInputSchema PreviewId { get; } = CopilotToolInputSchema.FromJsonSchema(
            JsonSerializer.SerializeToElement(new Dictionary<string, object?>
            {
                ...
                ["properties"] = new Dictionary<string, object?>
                {
                    ["previewId"] = new { type = "string", description = "Exact preview_id returned by PreviewWorkspacePatch." },
                },
    ...
    public sealed class CopilotRollbackWorkspacePatchTool : ICopilotFrameworkApprovedTool, ICopilotFrameworkApprovalPresentation
    {
        ...
        public string Description => "Roll back an applied workspace patch using its preview_id. Requires native approval and restores bytes only when the file still matches the applied patch SHA-256.";
