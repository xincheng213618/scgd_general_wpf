    ...
    public static class ColorVisionServiceHostManager
    {
        ...
        internal static string CreateInstallScript()
        {
            return string.Join(Environment.NewLine, new[]
            {
                ...
                "        $serviceConfigExitCode = $LASTEXITCODE",
                "    } finally {",
                "        $service.Dispose()",
                "        $service = $null",
                "    }",
                "    if ($serviceConfigExitCode -eq 2) {",
                "        Write-Step \"Existing service registration could not be configured (error 2); recreating it.\"",
                "        & sc.exe delete $serviceName",
                "        $deleteExitCode = $LASTEXITCODE",
                "        if ($deleteExitCode -ne 0 -and $deleteExitCode -ne 1060 -and $deleteExitCode -ne 1072) { throw \"Failed to delete stale service registration: $deleteExitCode\" }",
                ...
                "                if ([DateTime]::UtcNow -lt $deleteDeadline) { Start-Sleep -Milliseconds 250 }",
                "            }",
                "        } while ($remainingService -and [DateTime]::UtcNow -lt $deleteDeadline)",
                "        $remainingService = Get-Service -Name $serviceName -ErrorAction SilentlyContinue",
                "        if ($remainingService) {",
                "            $remainingService.Dispose()",
                "            throw \"Stale service registration is still pending deletion. Restart Windows, then retry the installation.\"",
                "        }",
                "        & sc.exe create $serviceName binPath= ('\"' + $exe + '\"') start= auto DisplayName= $displayName",
                "        if ($LASTEXITCODE -ne 0) { throw \"Failed to recreate service: $LASTEXITCODE\" }",
                "        Write-Step \"Existing service registration was recreated after configuration error 2.\"",
                "    } elseif ($serviceConfigExitCode -ne 0) {",
                "        throw \"Failed to configure service: $serviceConfigExitCode\"",
                "    }",
                "} else {",
                "    & sc.exe create $serviceName binPath= ('\"' + $exe + '\"') start= auto DisplayName= $displayName",
                "    if ($LASTEXITCODE -ne 0) { throw \"Failed to create service: $LASTEXITCODE\" }",
                "}",
