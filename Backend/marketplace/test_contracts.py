...
These tests freeze the external interface contracts. Any rewrite must pass
all of these tests to guarantee backward compatibility.

Covers:
  - All public page routes
...
class PublicPageContracts(ContractTestBase):
    """Contract tests for public HTML pages."""
    ...
    def test_updates_page_returns_200(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("1.0.0.1", resp.get_data(as_text=True))
    ...
    def test_browse_subpath_returns_200(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertIn("SomeTool.exe", resp.get_data(as_text=True))

    def test_browse_nonexistent_returns_404(self):
        resp = self.client.get("/browse/nonexistent")
    ...
    def test_plugin_detail_page_404_for_missing(self):
        resp = self.client.get("/plugins/NoSuchPlugin")
        self.assertEqual(resp.status_code, 404)
...
class AuthContracts(ContractTestBase):
    ...
    def test_login_failure_returns_200_with_error(self):
        resp = self.client.post("/login", data={
            "username": "admin", "password": "wrong",
        })
        self.assertEqual(resp.status_code, 200)
        self.assertIn("错误", resp.get_data(as_text=True))
