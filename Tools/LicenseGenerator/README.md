...
这是一个用于生成 ColorVision 软件许可证的工具，提供了简单易用的图形界面，支持**简单许可证**和**增强型许可证**两种格式。
...
#### 生成简单许可证
3. 切换到"简单许可证"标签页
4. 可以直接使用当前机器码，或输入其他机器码
5. 点击"生成简单许可证"按钮
6. 生成的许可证可以复制到剪贴板或保存到文件
...
5. 点击"生成增强许可证"按钮
6. 系统会显示剩余有效天数，并自动验证许可证
7. 生成的许可证可以复制到剪贴板或保存为 .lic 文件
...
### 简单许可证验证
生成的简单许可证可以通过 `ColorVision.UI.ACE.License.Check()` 方法验证。
