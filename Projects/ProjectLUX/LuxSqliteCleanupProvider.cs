...
{
    public sealed class LuxSqliteCleanupProvider :
        ...
        IDatabaseCleanupMigrationProvider
    {
        ...
        public string Id => "projectlux-sqlite";
        public string DisplayName => "LUX SQLite";
        ...
        public string MigrationConfirmationMessage =>
            ...
            Environment.NewLine +
            "迁移后旧版插件不能读取这些历史 JSON；迁移过程中请勿执行 LUX 测试。此按钮仅用于现场数据库过渡，全部迁移完成后可移除。";
        ...
        public IReadOnlyList<DatabaseCleanupTableInfo> LoadTables()
        {
            ...
            resultTable.RowCount = db.Queryable<ProjectLUXReuslt>().Count();
        ...
        private static DatabaseCleanupExecutionResult CleanupHistoryCore(int keepMonths)
        {
            if (!File.Exists(ViewResultManager.SqliteDbPath))
            {
                ...
                {
                    StatusMessage = "LUX SQLite 数据库文件不存在。"
                };
            }
            ...
            try
            {
                deletedResultRows = db.Deleteable<ProjectLUXReuslt>().Where(item => item.CreateTime < cutoffDate).ExecuteCommand();
            ...
            TryVacuum(db);
            RefreshLuxWindowIfOpen();

            var result = new DatabaseCleanupExecutionResult
            {
                StatusMessage = $"已保留最近 {keepMonths} 个月的 LUX 数据。"
            };
        ...
        private static DatabaseCleanupExecutionResult CleanupAllCore()
        {
            ...
            try
            {
                deletedResultRows = db.Deleteable<ProjectLUXReuslt>().ExecuteCommand();
            ...
            var result = new DatabaseCleanupExecutionResult
            {
                StatusMessage = "已清空 LUX SQLite 结果表。"
            };
            result.SummaryLines.Add($"{ResultTableName}: 删除 {deletedResultRows:N0} 行");
            result.SummaryLines.Add($"{ObjectiveResultTableName}: 删除 {deletedObjectiveRows:N0} 行");
            return result;
        ...
        private static DatabaseCleanupBackupResult CreateBackupCore()
        {
            ...
            if (!File.Exists(databasePath))
                throw new FileNotFoundException("LUX SQLite 数据库文件不存在。", databasePath);

            string databaseDirectory = Path.GetDirectoryName(databasePath)
                ?? throw new InvalidOperationException("无法确定 LUX SQLite 数据库目录。");
            string backupDirectory = Path.Combine(databaseDirectory, "ProjectLUX.Backups");
            Directory.CreateDirectory(backupDirectory);
            string backupPath = Path.Combine(
                backupDirectory,
                $"ProjectLUX.backup-{DateTime.Now:yyyyMMdd_HHmmss_fff}.db");
            ...
            try
            {
                ...
                {
                    StatusMessage = $"已创建 LUX 完整备份（{FormatSize(new FileInfo(backupPath).Length)}）。",
                    FilePath = backupPath,
                };
            }
            catch
        ...
        private static void RefreshLuxWindowIfOpen()
        {
