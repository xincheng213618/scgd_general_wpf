                    ...
                    if (master.ImgFileType == ViewResultAlgType.POI_XYZ)
                    {
                        ...
                        foreach (var item in poiPoints)
                        {
                            ...
                            if (item.PoiName == Config.Key_Center)
                            {
                                ...
                                testResult.CenterCIE1931ChromaticCoordinatesy.Value = poi.y;
                        ...
                        double AverageLuminance = testResult.PoixyuvDatas.Average(x => x.Y);
                        testResult.AverageLuminance.LowLimit = recipeConfig.AverageLuminance.Min;
                        testResult.AverageLuminance.UpLimit = recipeConfig.AverageLuminance.Max;
                        testResult.AverageLuminance.Value = AverageLuminance;
                        testResult.AverageLuminance.TestValue = testResult.AverageLuminance.Value.ToString("F3") + "nit";
                    }
                    ...
                    if (master.ImgFileType == ViewResultAlgType.PoiAnalysis)
                    {
                        if (master.TName.Contains(Config.LuminanceUniformityTempName))
                        {
                            ...
                            if (details.Count == 1)
                            {
                                ...
                                view.PoiAnalysisResult.result.Value = view.PoiAnalysisResult.result.Value * fixConfig.LuminanceUniformity *100;
