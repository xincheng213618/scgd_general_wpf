...
namespace ProjectLUX
{
    ...
    public partial class LUXWindow : Window, IDisposable
    {
        ...
        public static LUXRecipeConfig SPECConfig => RecipeManager.RecipeConfig;
        LogOutput logOutput;
        private void Window_Initialized(object sender, EventArgs e)
        {
            ...
            string Name = "Default";
            if (RecipeManager.RecipeConfigs.TryGetValue(Name, out LUXRecipeConfig recipeConfig))
            {
                RecipeManager.RecipeConfig = recipeConfig;
            }
            else
            {
                recipeConfig = new LUXRecipeConfig();
        ...
        private void Processing(string SerialNumber)
        {
            ...
            else if (result.Model.Contains("Black"))
            {
                ...
                if (ViewResluts.FirstOrDefault(a => a.SN == ProjectLUXConfig.Instance.SN) is ProjectLUXReuslt result1)
                {
            ...
            else if (result.Model.Contains("Chessboard"))
            {
                ...
                foreach (var AlgResultMaster in AlgResultMasterlists)
                {
                    ...
                    //        string filePath = Path.Combine(ProjectLUXConfig.Instance.ResultSavePath, $"Chessboard_{timeStr}.csv");
            ...
            else if (result.Model.Contains("MTF_H"))
            {
                ...
                foreach (var AlgResultMaster in AlgResultMasterlists)
                {
                    if (AlgResultMaster.ImgFileType == ColorVision.Engine.ViewResultAlgType.MTF && AlgResultMaster.version == "2.0")
                    {
                        ...
                        {
                            string timeStr = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                            string filePath = Path.Combine(ProjectLUXConfig.Instance.ResultSavePath, $"MTF_H_{timeStr}.csv");
                            var csvBuilder = new StringBuilder();
            ...
            else if (result.Model.Contains("MTF_V"))
            {
                ...
                {
                    string timeStr = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                    string filePath = Path.Combine(ProjectLUXConfig.Instance.ResultSavePath, $"MTF_V_{timeStr}.csv");
                    var csvBuilder = new StringBuilder();
                    csvBuilder.AppendLine($"name,x,y,w,h,mtfValue");
