...
│
├── Recipe/                         # Recipe（限值 + 修正因子）系统
│   ├── RecipeBase.cs               #   Min/Max/Fix 判定与修正项
...
- **服务定位器** — `RecipeConfig.GetRequiredService<T>()` 获取 Recipe 类型化配置
- **单例管理器** — ProcessManager / RecipeManager / ViewResultManager / SummaryManager
- **反射发现** — ProcessManager 扫描 `IProcess` 实现，RecipeManager 扫描 Recipe 配置实现
...
| `ARVRRecipe.json` | `%APPDATA%/ColorVision/Config/` | Recipe 配置 — 各测试项的合格/不合格限值与修正因子系数 |
