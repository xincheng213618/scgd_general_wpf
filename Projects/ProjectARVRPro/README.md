...
| White 51 | `White51Process` | 旧版固定 W51 视场角测试 |
| Black | `BlackProcess` | 暗电流、FOFO 对比度（依赖 W255 结果） |
| 亮色度测试 | `LuminanceChromaticityProcess` | 按可配置 Key 输出亮度、色度与均匀性；W25 使用 `Key=W25`、`CenterKey=P_9`，Key 为 White 时同时写入 W255 兼容结果 |
...
主窗口底部状态栏的“测试”按钮会连续重复当前组的一键执行流程。运行中按钮显示已完成轮数并可点击停止；停止当前流程时会将未完成批次记录为已取消，已经完成的每轮结果仍按正常一键执行保存。
