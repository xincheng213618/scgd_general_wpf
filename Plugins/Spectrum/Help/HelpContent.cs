...
【电参数说明】
EQE 的公式只使用电流 I，不使用电压 V。电流用于计算每秒注入的电子数；电压仅用于计算电功率 V × I 和光效。

【使用场景】
在本软件的「光通量模式」下，输入非零电流后即可计算 EQE；同时输入电压后还可计算光效。"
            });
...
【与辐射强度的区别】
本软件此处显示的是辐射通量 Φe（W），表示光源向所有方向发出的总辐射功率，不是辐射强度。
辐射强度 Ie = dΦe/dΩ，表示指定方向上单位立体角内的辐射通量，单位为 W/sr；计算它还需要方向或立体角测量信息。"
            });
...
3. 方法二（自动）：点击校零旁的齿轮，在「自动校零方式」中选择快门或滤色轮
4. 选择快门时连接快门控制器；选择滤色轮时连接滤色轮，并配置实际的遮光孔位（0–4）
5. 勾选「自动校零」，测量前软件会自动完成遮光、校零和光路恢复
...
【自动校零功能】
勾选「自动校零」后，每次测量前会自动：
1. 快门模式：关闭快门；滤色轮模式：记住当前测量孔位并切到已配置的遮光孔位
2. 采集暗电流数据
3. 快门模式：重新打开快门；滤色轮模式：恢复原测量孔位
4. 确认光路恢复成功后进行正式测量

滤色轮遮光孔位必须按实际硬件配置，不能把默认的 Empty 孔位当成遮光孔位。切换或恢复失败时测量会停止，避免在错误光路下保存结果。"
            });
...
【输入参数】
• 电压 V（伏特）：LED 正向电压，用于计算电功率和光效
• 电流 I（毫安）：LED 正向电流，用于计算 EQE，也参与光效计算
  可手动输入，也可通过源表（SMU）自动读取。

【计算内容】
1. **EQE（外量子效率）**：光子数 / 电子数，只依赖光谱和电流，不使用电压
