        ...
        private static readonly IReadOnlyList<ConoscopeHelpEntry> Entries = new[]
        {
            Entry(
                ...
                Numbers(
                    ...
                    "图像上方工具条只负责缩放；关注点相关操作在图像下方工具条完成。",
                    "如果只是检查单个点，可对关注点右键直接计算。",
                    "如果要做综合色域或黑白对比度，到 分析 页签记录 R/G/B 或 白/黑。",
                    "结果会在独立结果窗口中显示，不再堆回主界面。"),
                Bullets(1,
                    "在图像下方工具条里开启 关注点圆，左键拖出圆形采样区域。"),
                Bullets(1,
                    "然后点击 计算色域 或 计算对比度。"),
                H("你需要先理解的两个概念"),
                Bullets(
                    "活动 View：主页和分析页这些窗口级操作都默认作用于当前活动标签页。",
                ...
                Bullets(
                    ...
                    "导出前忘了先切到正确的显示通道，结果导出的不是当前想看的数据。")),
            ...
            Entry(
                ...
                "采集,流程模板,测量相机,拍照,校正,复用当前视图,活动View,观察相机,主页",
                H("Ribbon 路径"),
                Bullets(
                    "主页：打开 CVCIE 图像、切换型号、打开观察相机，以及当前视图的 3D/CIE/导出入口。",
            ...
            Entry(
                ...
                H("Ribbon 路径"),
                Bullets(
                    "处理 -> 启用：控制打开图像时是否自动套用当前预处理预设。",
                    "处理 -> 显示：控制是否启用伪彩显示、是否限制到有效圆域，以及色带选择。",
                    "处理 -> 滤波：启用滤波后选择滤波类型，并直接编辑对应参数。",
                    "处理 -> 灰尘滤除：启用后直接设置灰尘类型、阈值、面积和修复半径。",
                    "处理 -> 应用：把当前预处理预设应用到活动 View。"),
                H("当前界面的职责分工"),
                Bullets(
                    "启用预处理 是打开时自动应用当前预设的总开关。",
                    "显示 负责控制伪彩、有效圆域限制和色带，不改变底层 XYZ 数据文件。",
                    "滤波 负责选择主滤波模式，例如低通、移动平均、高斯、中值、双边，并显示对应参数。",
                    "灰尘滤除 作为独立模块启用，只有启用后才展开对应配置。",
                    "设置 会打开完整参数窗口，适合做更细的集中检查。",
                    "应用 用来立即把当前预设作用到当前活动 View。"),
                H("适合的使用顺序"),
                Numbers(
                    "先在显示通道中确认你要观察的是 Y、x/y/u/v 还是色差。",
                    "再决定是否开启伪彩和有效圆域限制。",
                    "需要抑制噪声时开启滤波，并只调整当前滤波类型对应的参数。",
                    "对灰尘、亮点、局部污染敏感的图像，再启用灰尘滤除并微调阈值和面积范围。",
            ...
            Entry(
                ...
                Numbers(
                    ...
                    "需要回到编辑选择态时，取消 圆 或 橡皮 按钮即可；不再单独显示 编辑 按钮。",
                    "需要清空全部关注点时，把 POI 模板切回 Empty。",
            ...
            Entry(
                ...
                "CIE、3D 和曲线导出都收口到主页当前视图入口；方位角和极角导出直接使用当前显示通道。",
                "CIE,3D,曲线,导出,方位角,极角,高级导出,当前视图",
                H("3D 与 CIE"),
                Bullets(
                    "主页 -> 视图功能 -> 3D：打开当前活动 View 的 3D 视图。",
                    "主页 -> 视图功能 -> CIE：打开当前活动 View 的 CIE 色度图窗口。",
                    "这些入口现在属于主窗口当前视图功能区，不再显示在图像 View 顶部工具条里。"),
                H("曲线显示"),
                Bullets(
                    "右侧下方面板是 参考曲线。",
                ...
                Bullets(
                    "主页 -> 当前视图：先切到要导出的显示通道。",
                    "主页 -> 当前通道导出：执行方位模式导出和极角模式导出。",
                    "主页 -> 当前通道导出 -> 高级导出：批量导出多个通道、多个步长或截面数据。"),
                H("使用建议"),
                Bullets(
            ...
            Entry(
                ...
                "3D 使用当前显示通道生成高度图；CIE 有主页当前视图查看和结果窗叠图两种入口。",
